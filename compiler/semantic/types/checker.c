...
static void zt_transfer_stack_init(zt_transfer_stack *stack) {
...
static void zt_transfer_stack_dispose(zt_transfer_stack *stack) {
    size_t i;
...
static int zt_transfer_stack_contains(const zt_transfer_stack *stack, const char *type_name) {
    size_t i;

    if (stack == NULL || type_name == NULL) return 0;
    for (i = 0; i < stack->count; i++) {
        if (stack->items[i] != NULL && strcmp(stack->items[i], type_name) == 0) {
            return 1;
        }
    }
    return 0;
...
static int zt_transfer_stack_push(zt_transfer_stack *stack, const char *type_name) {
    char **new_items;
    size_t new_capacity;
    ...
    if (stack->count >= stack->capacity) {
        ...
        new_items = (char **)realloc(stack->items, new_capacity * sizeof(char *));
        if (new_items == NULL) return 0;
        stack->items = new_items;
        stack->capacity = new_capacity;
    }

    stack->items[stack->count] = zt_checker_strdup(type_name);
    if (stack->items[stack->count] == NULL) return 0;
    stack->count += 1;
    return 1;
}

static void zt_transfer_stack_pop(zt_transfer_stack *stack) {
    ...
    for (i = 0; i < root->as.file.declarations.count; i++) {
        ...
        switch (decl->kind) {
            ...
            case ZT_AST_TYPE_ALIAS_DECL:
                zt_decl_list_push(&catalog->decls, decl->as.type_alias_decl.name, decl);
                break;
            case ZT_AST_CONST_DECL:
                zt_decl_list_push(&catalog->decls, decl->as.const_decl.name, decl);
                catalog->value_decl_count += 1;
                break;
            case ZT_AST_VAR_DECL:
                zt_decl_list_push(&catalog->decls, decl->as.var_decl.name, decl);
                catalog->value_decl_count += 1;
                break;
...
static int zt_checker_decl_is_public(const zt_ast_node *decl) {
    ...
    switch (decl->kind) {
        ...
        case ZT_AST_TYPE_ALIAS_DECL:
            return decl->as.type_alias_decl.is_public;
        case ZT_AST_CONST_DECL:
            return decl->as.const_decl.is_public;
        case ZT_AST_VAR_DECL:
            return decl->as.var_decl.is_public;
        case ZT_AST_EXTERN_DECL:
...
static zt_type *zt_builtin_type_by_name(const char *name) {
    ...
    if (strcmp(name, "u8") == 0) return zt_type_make(ZT_TYPE_UINT8);
    if (strcmp(name, "u16") == 0) return zt_type_make(ZT_TYPE_UINT16);
    if (strcmp(name, "u32") == 0) return zt_type_make(ZT_TYPE_UINT32);
    if (strcmp(name, "u64") == 0) return zt_type_make(ZT_TYPE_UINT64);
...
static void zt_checker_append_text(char *buffer, size_t buffer_size, const char *text) {
    size_t used;
    size_t remaining;
    size_t len;

    if (buffer == NULL || buffer_size == 0 || text == NULL) return;
    used = strlen(buffer);
    if (used >= buffer_size - 1) return;
    remaining = buffer_size - 1 - used;
    len = strlen(text);
...
static void zt_checker_diag_callable_signature(zt_checker *checker, zt_source_span span, const char *name, const zt_type *expected, const zt_type *received) {
    char expected_buf[256];
    char received_buf[256];

    zt_type_describe(expected, expected_buf, sizeof(expected_buf));
    zt_type_describe(received, received_buf, sizeof(received_buf));
    zt_diag_list_add(
        &checker->result->diagnostics,
        ZT_DIAG_CALLABLE_SIGNATURE_MISMATCH,
        span,
        "callable '%s' signature mismatch: expected %s but received %s",
        name != NULL ? name : "<callable>",
        expected_buf,
        received_buf);
}

static void zt_checker_diag_invalid_callable_ref_expected(zt_checker *checker, zt_source_span span, const char *kind, const char *name, const zt_type *expected) {
    char expected_buf[256];
    ...
    zt_diag_list_add(
        ...
        span,
        "%s '%s' cannot be used as a callable value; expected %s. Create a non-generic top-level wrapper with this exact signature",
        kind != NULL ? kind : "function",
        name != NULL ? name : "<anon>",
        expected_buf);
}

static void zt_checker_format_missing_enum_cases(const zt_ast_node *enum_decl, const int *seen_variants, char *buffer, size_t buffer_size) {
    size_t i;
    int first = 1;

    if (buffer == NULL || buffer_size == 0) return;
    buffer[0] = '\0';
    if (enum_decl == NULL || enum_decl->kind != ZT_AST_ENUM_DECL || seen_variants == NULL) return;

    for (i = 0; i < enum_decl->as.enum_decl.variants.count; i++) {
        const zt_ast_node *variant = enum_decl->as.enum_decl.variants.items[i];
        if (variant == NULL || seen_variants[i]) continue;
        if (!first) {
            zt_checker_append_text(buffer, buffer_size, ", ");
        }
        zt_checker_append_text(buffer, buffer_size, enum_decl->as.enum_decl.name);
        zt_checker_append_text(buffer, buffer_size, ".");
        zt_checker_append_text(buffer, buffer_size, variant->as.enum_variant.name);
...
static void zt_checker_add_dyn_static_dispatch_note(zt_checker *checker, zt_diag_code code, zt_source_span span, const char *trait_name) {
    zt_diag_list_add_severity(
        &checker->result->diagnostics,
        code,
        ZT_DIAG_SEVERITY_NOTE,
        span,
        "if dynamic dispatch is not required, prefer a generic parameter with a where constraint instead of dyn<%s>",
        trait_name != NULL ? trait_name : "Trait");
}
...
static int zt_type_is_core_trait_name(const char *name) {
    return name != NULL &&
           (strcmp(name, "Equatable") == 0 ||
            ...
            strcmp(name, "Comparable") == 0);
}

static int zt_checker_split_prefix(const char *name, char *prefix, size_t prefix_capacity) {
    const char *dot;
    size_t len;
...
static int zt_checker_type_is_transferable_inner(zt_checker *checker, zt_binding_scope *scope, const zt_type *type, zt_transfer_stack *stack);

/* R3.M4: Validate that a trait can be used with dyn<Trait> per the minimum subset rules.
 * Returns 1 if valid, 0 if invalid (diagnostic emitted). */
static int zt_checker_validate_dyn_trait(zt_checker *checker, const zt_ast_node *trait_decl_node, zt_source_span use_span) {
    const zt_ast_node *method;
    size_t i, p;

    if (trait_decl_node == NULL || trait_decl_node->kind != ZT_AST_TRAIT_DECL) {
        return 0;
    }

    /* Rule: no generic traits */
    if (trait_decl_node->as.trait_decl.type_params.count > 0) {
        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_DYN_GENERIC_TRAIT, use_span,
            "trait '%s' cannot be used as dyn<%s>: generic traits are not dyn-safe",
            trait_decl_node->as.trait_decl.name, trait_decl_node->as.trait_decl.name);
        zt_checker_add_dyn_static_dispatch_note(checker, ZT_DIAG_DYN_GENERIC_TRAIT, use_span, trait_decl_node->as.trait_decl.name);
        return 0;
    }

    /* Rule: max 8 methods */
    if (trait_decl_node->as.trait_decl.methods.count > 8) {
        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_DYN_TOO_MANY_METHODS, use_span,
            "trait '%s' cannot be used as dyn<%s>: it has %zu methods, but dyn supports at most 8",
            trait_decl_node->as.trait_decl.name,
            trait_decl_node->as.trait_decl.name, trait_decl_node->as.trait_decl.methods.count);
        zt_checker_add_dyn_static_dispatch_note(checker, ZT_DIAG_DYN_TOO_MANY_METHODS, use_span, trait_decl_node->as.trait_decl.name);
        return 0;
    }

    /* Rule: no mut methods, all params/returns must be copyable */
    for (i = 0; i < trait_decl_node->as.trait_decl.methods.count; i++) {
        method = trait_decl_node->as.trait_decl.methods.items[i];
        if (method == NULL) continue;

        /* Rule: no mut func */
        if (method->as.trait_method.is_mutating) {
            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_DYN_MUT_METHOD, use_span,
                "trait '%s' cannot be used as dyn<%s>: mut method '%s' needs a concrete receiver",
                trait_decl_node->as.trait_decl.name,
                trait_decl_node->as.trait_decl.name,
                method->as.trait_method.name);
            zt_checker_add_dyn_static_dispatch_note(checker, ZT_DIAG_DYN_MUT_METHOD, use_span, trait_decl_node->as.trait_decl.name);
            return 0;
        }

        /* Rule: params must be copyable */
        for (p = 0; p < method->as.trait_method.params.count; p++) {
            const zt_ast_node *param = method->as.trait_method.params.items[p];
            if (param != NULL && param->kind == ZT_AST_PARAM && param->as.param.type_node != NULL) {
                zt_type *param_type = zt_checker_resolve_type(checker, param->as.param.type_node, NULL);
                int copyable = 0;
                if (param_type != NULL) {
                    int is_scalar = (param_type->kind >= ZT_TYPE_INT8 && param_type->kind <= ZT_TYPE_UINT64) ||
                                    param_type->kind == ZT_TYPE_INT || param_type->kind == ZT_TYPE_FLOAT;
                    copyable = is_scalar ||
                               param_type->kind == ZT_TYPE_TEXT ||
                               param_type->kind == ZT_TYPE_BYTES ||
                               param_type->kind == ZT_TYPE_BOOL;
                }
                if (!copyable) {
                    char param_type_buf[256];
                    zt_type_describe(param_type, param_type_buf, sizeof(param_type_buf));
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_DYN_UNCOPYABLE, use_span,
                        "trait '%s' cannot be used as dyn<%s>: method '%s' has non-copyable parameter type %s",
                        trait_decl_node->as.trait_decl.name,
                        trait_decl_node->as.trait_decl.name,
                        method->as.trait_method.name,
                        param_type_buf);
                    zt_checker_add_dyn_static_dispatch_note(checker, ZT_DIAG_DYN_UNCOPYABLE, use_span, trait_decl_node->as.trait_decl.name);
        ...
        /* Rule: return type must be copyable */
        if (method->as.trait_method.return_type != NULL) {
            zt_type *ret_type = zt_checker_resolve_type(checker, method->as.trait_method.return_type, NULL);
            int copyable = 0;
            if (ret_type != NULL) {
                int is_scalar = (ret_type->kind >= ZT_TYPE_INT8 && ret_type->kind <= ZT_TYPE_UINT64) ||
                                ret_type->kind == ZT_TYPE_INT || ret_type->kind == ZT_TYPE_FLOAT;
                copyable = is_scalar ||
                           ret_type->kind == ZT_TYPE_TEXT ||
                           ret_type->kind == ZT_TYPE_BYTES ||
                           ret_type->kind == ZT_TYPE_BOOL ||
                           ret_type->kind == ZT_TYPE_VOID;
            }
            if (!copyable) {
                char ret_type_buf[256];
                zt_type_describe(ret_type, ret_type_buf, sizeof(ret_type_buf));
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_DYN_UNCOPYABLE, use_span,
                    "trait '%s' cannot be used as dyn<%s>: method '%s' returns non-copyable type %s",
                    trait_decl_node->as.trait_decl.name,
                    trait_decl_node->as.trait_decl.name,
                    method->as.trait_method.name,
                    ret_type_buf);
                zt_checker_add_dyn_static_dispatch_note(checker, ZT_DIAG_DYN_UNCOPYABLE, use_span, trait_decl_node->as.trait_decl.name);
                zt_type_dispose(ret_type);
                return 0;
            }
            zt_type_dispose(ret_type);
        }
...
static zt_type *zt_checker_resolve_user_type(zt_checker *checker, const char *name, zt_type_list args, zt_source_span span, zt_binding_scope *scope) {
    ...
    if (zt_name_has_dot(name)) {
        ...
        if (decl->kind == ZT_AST_TYPE_ALIAS_DECL) {
            if (args.count != 0) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, span, "type alias '%s' does not accept type arguments", name);
                zt_type_list_dispose(&args);
                return zt_type_make(ZT_TYPE_UNKNOWN);
            }
            zt_type_list_dispose(&args);
            return zt_checker_resolve_type(checker, decl->as.type_alias_decl.target_type, scope);
        }
    ...
    if (decl == NULL) {
        if (strcmp(name, "string") == 0) {
            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, span, "unknown type 'string'; use 'text' for text values");
            zt_type_list_dispose(&args);
            return zt_type_make(ZT_TYPE_UNKNOWN);
        }
        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, span, "unknown type '%s'", name);
...
static zt_type *zt_checker_resolve_type(zt_checker *checker, const zt_ast_node *node, zt_binding_scope *scope) {
    ...
    if (node->kind == ZT_AST_TYPE_SIMPLE) {
        ...
        return zt_checker_resolve_user_type(checker, name, zt_type_list_make(), node->span, scope);
    }

    if (node->kind == ZT_AST_TYPE_CALLABLE) {
        zt_type_list params = zt_type_list_make();
        zt_type *return_type = NULL;
        size_t pi;
        for (pi = 0; pi < node->as.type_callable.params.count; pi++) {
            zt_type *pt = zt_checker_resolve_type(checker, node->as.type_callable.params.items[pi], scope);
            /* Phase 1 rejects container element callables. Reject callable
             * parameter whose own kind is callable? Callables-of-callables
             * are allowed as parameters; only container element positions
             * are blocked, enforced separately when resolving list/map/etc.
             */
            zt_type_list_push(&params, pt);
        }
        return_type = zt_checker_resolve_type(checker, node->as.type_callable.return_type, scope);
        return zt_type_make_callable(return_type, params);
    }
    ...
    if (node->kind == ZT_AST_TYPE_DYN) {
        ...
        /* R3.M4: Validate dyn<Trait> minimum subset rules */
        {
            const char *trait_name = NULL;
            const zt_ast_node *inner = node->as.type_dyn.inner_type;
            if (inner != NULL && inner->kind == ZT_AST_TYPE_SIMPLE) {
                trait_name = inner->as.type_simple.name;
            } else if (inner != NULL && inner->kind == ZT_AST_TYPE_GENERIC) {
                trait_name = inner->as.type_generic.name;
            }
            if (trait_name != NULL) {
                const zt_ast_node *trait_decl = zt_catalog_find_decl(&checker->catalog, trait_name);
                if (trait_decl != NULL) {
                    zt_checker_validate_dyn_trait(checker, trait_decl, node->span);
                }
            }
        }
    ...
    if (expected_arity >= 0) {
        ...
        /* R3.M5 Phase 2: callable cannot appear as a container element type. */
        {
            size_t ci;
            for (ci = 0; ci < args.count; ci++) {
                if (args.items[ci] != NULL && args.items[ci]->kind == ZT_TYPE_CALLABLE) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_CALLABLE_ESCAPE_CONTAINER, node->span,
                        "callable type cannot be used as an argument of '%s'", name);
                }
            }
        }
        ...
        if (strcmp(name, "dyn") == 0 || strcmp(name, "any") == 0) return zt_type_make_with_args(ZT_TYPE_DYN, NULL, args);
        ...
        if (strcmp(name, "set") == 0) {
            zt_type *set_type = zt_type_make_with_args(ZT_TYPE_SET, NULL, args);
            if (set_type->args.count >= 1 &&
                (!zt_checker_type_implements_trait(checker, scope, set_type->args.items[0], "Hashable") ||
                 !zt_checker_type_implements_trait(checker, scope, set_type->args.items[0], "Equatable"))) {
                char key_buf[256];
                zt_type_describe(set_type->args.items[0], key_buf, sizeof(key_buf));
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_MAP_KEY_TYPE, node->span, "set element type '%s' must implement Hashable and Equatable", key_buf);
            }
            return set_type;
        }
    }
    ...
    return zt_checker_resolve_user_type(checker, name, args, node->span, scope);
}
    ...
    switch (type->kind) {
        ...
        case ZT_TYPE_UINT64:
            return strcmp(trait_name, "Equatable") == 0 ||
                   strcmp(trait_name, "Hashable") == 0 ||
                   strcmp(trait_name, "TextRepresentable") == 0 ||
                   strcmp(trait_name, "Comparable") == 0;
        case ZT_TYPE_FLOAT:
...
static int zt_checker_infer_closure_return_from_stmt(
        zt_checker *checker,
        const zt_ast_node *stmt,
        zt_binding_scope *scope,
        zt_function_context *fn_ctx,
        zt_type **inferred_type);

static void zt_checker_record_inferred_return_type(zt_type **inferred_type, const zt_type *candidate) {
    if (inferred_type == NULL || candidate == NULL) return;
    if (*inferred_type == NULL || ((*inferred_type)->kind == ZT_TYPE_UNKNOWN && candidate->kind != ZT_TYPE_UNKNOWN)) {
        zt_type_dispose(*inferred_type);
        *inferred_type = zt_type_clone(candidate);
    }
}

static void zt_checker_bind_declared_local_type(
        zt_checker *checker,
        const zt_ast_node *stmt,
        zt_binding_scope *scope,
        int is_mutable) {
    ...
    if (checker == NULL || stmt == NULL || scope == NULL) return;
    if (stmt->kind == ZT_AST_CONST_DECL) {
        name = stmt->as.const_decl.name;
        decl_type = zt_checker_resolve_type(checker, stmt->as.const_decl.type_node, scope);
    } else if (stmt->kind == ZT_AST_VAR_DECL) {
    ...
    zt_binding_scope_declare(scope, ZT_BINDING_VALUE, name, zt_type_clone(decl_type), is_mutable);
    zt_type_dispose(decl_type);
}

static int zt_checker_infer_closure_return_from_block(
        zt_checker *checker,
        const zt_ast_node *block,
        zt_binding_scope *parent_scope,
        zt_function_context *fn_ctx,
        zt_type **inferred_type) {
    ...
    if (block == NULL || block->kind != ZT_AST_BLOCK) return 0;

    zt_binding_scope_init(&local_scope, parent_scope);
    for (i = 0; i < block->as.block.statements.count; i++) {
        found |= zt_checker_infer_closure_return_from_stmt(
            checker,
...
static int zt_checker_infer_closure_return_from_stmt(
    ...
    switch (stmt->kind) {
        case ZT_AST_BLOCK:
            return zt_checker_infer_closure_return_from_block(checker, stmt, scope, fn_ctx, inferred_type);
        case ZT_AST_RETURN_STMT:
            if (stmt->as.return_stmt.value != NULL) {
                expr_info = zt_checker_check_expression(checker, stmt->as.return_stmt.value, scope, fn_ctx, NULL);
                zt_checker_record_inferred_return_type(inferred_type, expr_info.type);
                zt_expr_info_dispose(&expr_info);
            } else {
                zt_type *void_type = zt_type_make(ZT_TYPE_VOID);
                zt_checker_record_inferred_return_type(inferred_type, void_type);
                zt_type_dispose(void_type);
            }
            return 1;
        case ZT_AST_CONST_DECL:
            zt_checker_bind_declared_local_type(checker, stmt, scope, 0);
            return 0;
        case ZT_AST_VAR_DECL:
            zt_checker_bind_declared_local_type(checker, stmt, scope, 1);
            return 0;
        case ZT_AST_IF_STMT:
            found |= zt_checker_infer_closure_return_from_block(checker, stmt->as.if_stmt.then_block, scope, fn_ctx, inferred_type);
            if (stmt->as.if_stmt.else_block != NULL) {
                found |= zt_checker_infer_closure_return_from_stmt(checker, stmt->as.if_stmt.else_block, scope, fn_ctx, inferred_type);
            }
            return found;
        case ZT_AST_WHILE_STMT:
            return zt_checker_infer_closure_return_from_block(checker, stmt->as.while_stmt.body, scope, fn_ctx, inferred_type);
        case ZT_AST_REPEAT_STMT:
            return zt_checker_infer_closure_return_from_block(checker, stmt->as.repeat_stmt.body, scope, fn_ctx, inferred_type);
        case ZT_AST_FOR_STMT:
            expr_info = zt_checker_check_expression(checker, stmt->as.for_stmt.iterable, scope, fn_ctx, NULL);
            item_type = zt_type_make(ZT_TYPE_UNKNOWN);
            second_type = zt_type_make(ZT_TYPE_INT);
            if (expr_info.type != NULL) {
                if (expr_info.type->kind == ZT_TYPE_LIST && expr_info.type->args.count > 0) {
                    zt_type_dispose(item_type);
                    item_type = zt_type_clone(expr_info.type->args.items[0]);
                } else if (expr_info.type->kind == ZT_TYPE_TEXT) {
                    zt_type_dispose(item_type);
                    item_type = zt_type_make(ZT_TYPE_TEXT);
                } else if (expr_info.type->kind == ZT_TYPE_SET && expr_info.type->args.count > 0) {
                    zt_type_dispose(item_type);
                    item_type = zt_type_clone(expr_info.type->args.items[0]);
                } else if (expr_info.type->kind == ZT_TYPE_MAP && expr_info.type->args.count > 1) {
                    zt_type_dispose(item_type);
                    zt_type_dispose(second_type);
                    item_type = zt_type_clone(expr_info.type->args.items[0]);
                    second_type = zt_type_clone(expr_info.type->args.items[1]);
                }
            }
            zt_binding_scope_init(&nested_scope, scope);
            zt_binding_scope_declare(&nested_scope, ZT_BINDING_VALUE, stmt->as.for_stmt.item_name, zt_type_clone(item_type), 1);
            if (stmt->as.for_stmt.index_name != NULL) {
                zt_binding_scope_declare(&nested_scope, ZT_BINDING_VALUE, stmt->as.for_stmt.index_name, zt_type_clone(second_type), 1);
            }
            found = zt_checker_infer_closure_return_from_block(checker, stmt->as.for_stmt.body, &nested_scope, fn_ctx, inferred_type);
            zt_binding_scope_dispose(&nested_scope);
            zt_type_dispose(item_type);
            zt_type_dispose(second_type);
            zt_expr_info_dispose(&expr_info);
            return found;
        case ZT_AST_USING_STMT:
            expr_info = zt_checker_check_expression(checker, stmt->as.using_stmt.init_value, scope, fn_ctx, NULL);
            zt_binding_scope_init(&nested_scope, scope);
            if (expr_info.type != NULL) {
                zt_binding_scope_declare(&nested_scope, ZT_BINDING_VALUE, stmt->as.using_stmt.name, zt_type_clone(expr_info.type), 0);
            }
            if (stmt->as.using_stmt.body != NULL) {
                found = zt_checker_infer_closure_return_from_block(checker, stmt->as.using_stmt.body, &nested_scope, fn_ctx, inferred_type);
            }
            zt_binding_scope_dispose(&nested_scope);
            zt_expr_info_dispose(&expr_info);
            return found;
        case ZT_AST_MATCH_STMT:
            for (i = 0; i < stmt->as.match_stmt.cases.count; i++) {
                const zt_ast_node *case_node = stmt->as.match_stmt.cases.items[i];
                if (case_node == NULL || case_node->kind != ZT_AST_MATCH_CASE) continue;
                found |= zt_checker_infer_closure_return_from_block(checker, case_node->as.match_case.body, scope, fn_ctx, inferred_type);
            }
            return found;
        default:
            return 0;
...
static zt_type *zt_checker_make_std_concurrent_copy_type(const char *member) {
    zt_type_list args;

    if (member == NULL) return NULL;

    if (strcmp(member, "copy_int") == 0) return zt_type_make(ZT_TYPE_INT);
    if (strcmp(member, "copy_bool") == 0) return zt_type_make(ZT_TYPE_BOOL);
    if (strcmp(member, "copy_float") == 0) return zt_type_make(ZT_TYPE_FLOAT);
    if (strcmp(member, "copy_text") == 0) return zt_type_make(ZT_TYPE_TEXT);
    if (strcmp(member, "copy_bytes") == 0) return zt_type_make(ZT_TYPE_BYTES);

    if (strcmp(member, "copy_list_int") == 0) {
        args = zt_type_list_make();
        zt_type_list_push(&args, zt_type_make(ZT_TYPE_INT));
        return zt_type_make_with_args(ZT_TYPE_LIST, NULL, args);
    }

    if (strcmp(member, "copy_list_text") == 0) {
        args = zt_type_list_make();
        zt_type_list_push(&args, zt_type_make(ZT_TYPE_TEXT));
        return zt_type_make_with_args(ZT_TYPE_LIST, NULL, args);
    }

    if (strcmp(member, "copy_map_text_text") == 0) {
        args = zt_type_list_make();
        zt_type_list_push(&args, zt_type_make(ZT_TYPE_TEXT));
        zt_type_list_push(&args, zt_type_make(ZT_TYPE_TEXT));
        return zt_type_make_with_args(ZT_TYPE_MAP, NULL, args);
    }
...
static const zt_ast_node_list *zt_decl_type_params(const zt_ast_node *decl) {
    if (decl == NULL) return NULL;
    switch (decl->kind) {
        case ZT_AST_STRUCT_DECL: return &decl->as.struct_decl.type_params;
        case ZT_AST_ENUM_DECL: return &decl->as.enum_decl.type_params;
        default: return NULL;
    }
}

static void zt_checker_bind_decl_type_params(zt_binding_scope *scope, const zt_ast_node *decl, const zt_type *concrete_type) {
    const zt_ast_node_list *type_params;
    size_t i;

    if (scope == NULL || decl == NULL || concrete_type == NULL) return;
    type_params = zt_decl_type_params(decl);
    if (type_params == NULL) return;

    for (i = 0; i < type_params->count && i < concrete_type->args.count; i++) {
        const zt_ast_node *param = type_params->items[i];
        if (param != NULL && param->kind == ZT_AST_TYPE_SIMPLE) {
            zt_binding_scope_declare(
                scope,
                ZT_BINDING_TYPE_PARAM,
                param->as.type_simple.name,
                zt_type_clone(concrete_type->args.items[i]),
                0);
        }
    }
}
...
static int zt_checker_type_is_transferable_user_decl(
        zt_checker *checker,
        zt_binding_scope *scope,
        const zt_type *type,
        const zt_ast_node *decl,
        zt_transfer_stack *stack) {
    zt_binding_scope type_scope;
    char type_name[256];
    size_t i;
    int is_transferable = 1;

    if (checker == NULL || type == NULL || decl == NULL || stack == NULL) return 0;

    zt_type_format(type, type_name, sizeof(type_name));
    if (zt_transfer_stack_contains(stack, type_name)) {
        return 1;
    }
    if (!zt_transfer_stack_push(stack, type_name)) {
        return 0;
    }

    zt_binding_scope_init(&type_scope, scope);
    zt_checker_bind_decl_type_params(&type_scope, decl, type);
    ...
    if (decl->kind == ZT_AST_STRUCT_DECL) {
        for (i = 0; i < decl->as.struct_decl.fields.count; i++) {
            ...
            field_type = zt_checker_resolve_type(checker, field->as.struct_field.type_node, &type_scope);
            if (!zt_checker_type_is_transferable_inner(checker, &type_scope, field_type, stack)) {
                is_transferable = 0;
            }
            zt_type_dispose(field_type);
            if (!is_transferable) break;
        }
        ...
        for (i = 0; i < decl->as.enum_decl.variants.count; i++) {
            ...
            for (f = 0; f < variant->as.enum_variant.fields.count; f++) {
                ...
                const zt_ast_node *field_type_node = zt_checker_enum_variant_field_type_node(field);
                zt_type *field_type = zt_checker_resolve_type(checker, field_type_node, &type_scope);
                if (!zt_checker_type_is_transferable_inner(checker, &type_scope, field_type, stack)) {
                    is_transferable = 0;
                }
                zt_type_dispose(field_type);
    ...
    zt_transfer_stack_pop(stack);
    return is_transferable;
}

static int zt_checker_type_is_transferable_inner(
        zt_checker *checker,
        zt_binding_scope *scope,
        const zt_type *type,
        zt_transfer_stack *stack) {
    ...
    switch (type->kind) {
        ...
        case ZT_TYPE_SET:
            return type->args.count == 1 &&
                   zt_checker_type_is_transferable_inner(checker, scope, type->args.items[0], stack);
        case ZT_TYPE_RESULT:
        case ZT_TYPE_MAP:
            return type->args.count == 2 &&
                   zt_checker_type_is_transferable_inner(checker, scope, type->args.items[0], stack) &&
                   zt_checker_type_is_transferable_inner(checker, scope, type->args.items[1], stack);
        case ZT_TYPE_TYPE_PARAM:
            binding = zt_binding_scope_lookup(scope, type->name, ZT_BINDING_TYPE_PARAM);
            if (binding == NULL || binding->type == NULL || binding->type == type) return 0;
            return zt_checker_type_is_transferable_inner(checker, scope, binding->type, stack);
        case ZT_TYPE_USER:
            if (checker == NULL || type->name == NULL) return 0;
            decl = zt_catalog_find_decl(&checker->catalog, type->name);
            if (decl == NULL) return 0;
            return zt_checker_type_is_transferable_user_decl(checker, scope, type, decl, stack);
...
static const zt_ast_node *zt_checker_lookup_import_member(
        const zt_checker *checker,
        const zt_ast_node *object_expr,
        const char *field_name,
        char *combined_name,
        size_t combined_name_size,
        const char **out_alias,
        const char **out_import_path) {
    const char *alias;

    if (out_alias != NULL) *out_alias = NULL;
    if (out_import_path != NULL) *out_import_path = NULL;
    if (combined_name != NULL && combined_name_size > 0) combined_name[0] = '\0';

    if (checker == NULL ||
        object_expr == NULL ||
        object_expr->kind != ZT_AST_IDENT_EXPR ||
        object_expr->as.ident_expr.name == NULL ||
        field_name == NULL) {
        return NULL;
    }

    alias = object_expr->as.ident_expr.name;
    if (out_alias != NULL) *out_alias = alias;
    if (out_import_path != NULL) *out_import_path = zt_catalog_import_path_for_alias(&checker->catalog, alias);

    if (combined_name != NULL && combined_name_size > 0) {
        snprintf(combined_name, combined_name_size, "%s.%s", alias, field_name);
        return zt_catalog_find_decl(&checker->catalog, combined_name);
    }
...
static zt_expr_info zt_checker_check_field_expr(zt_checker *checker, const zt_ast_node *node, zt_binding_scope *scope, zt_function_context *fn_ctx) {
    ...
    if (node->as.field_expr.object != NULL &&
        ...
        member_decl = zt_checker_lookup_import_member(
            checker,
            node->as.field_expr.object,
            node->as.field_expr.field_name,
            combined_name,
        ...
        if (member_decl != NULL) {
            ...
            } else if (member_decl->kind == ZT_AST_VAR_DECL) {
                zt_type_dispose(result.type);
                result.type = zt_checker_resolve_type(checker, member_decl->as.var_decl.type_node, scope);
            } else {
...
static zt_expr_info zt_checker_check_enum_dot_expr(
        zt_checker *checker,
        const zt_ast_node *node,
        zt_binding_scope *scope,
        zt_function_context *fn_ctx,
        const zt_type *expected_type,
        const zt_ast_node *call_expr) {
    zt_expr_info result = zt_expr_info_make(expected_type != NULL ? zt_type_clone(expected_type) : zt_type_make(ZT_TYPE_UNKNOWN));
    const char *variant_name = node != NULL ? node->as.enum_dot_expr.variant_name : NULL;
    const char *enum_name = zt_type_base_name(expected_type);
    const zt_ast_node *enum_decl = NULL;
    const zt_ast_node *variant_decl = NULL;
    size_t positional_count = call_expr != NULL ? call_expr->as.call_expr.positional_args.count : 0;
    size_t named_count = call_expr != NULL ? call_expr->as.call_expr.named_args.count : 0;
    size_t field_count = 0;
    size_t arg_index;
    int *used_fields = NULL;

    if (expected_type == NULL || expected_type->kind != ZT_TYPE_USER || enum_name == NULL) {
        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "enum dot shorthand requires an expected enum type");
        return result;
    }

    enum_decl = zt_catalog_find_decl(&checker->catalog, enum_name);
    if (enum_decl == NULL || enum_decl->kind != ZT_AST_ENUM_DECL) {
        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "enum dot shorthand expected enum type '%s'", enum_name);
        return result;
    }

    variant_decl = zt_checker_find_enum_variant_decl(enum_decl, variant_name, NULL);
    if (variant_decl == NULL) {
        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, node->span, "unknown enum variant '%s.%s'", enum_name, variant_name != NULL ? variant_name : "<variant>");
        return result;
    }

    field_count = variant_decl->as.enum_variant.fields.count;
    if (call_expr == NULL && field_count > 0) {
        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, node->span, "enum variant '%s.%s' has payload and requires a constructor call", enum_name, variant_name);
        return result;
    }
    ...
    if (field_count > 0) {
        ...
        if (used_fields == NULL) {
            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, call_expr != NULL ? call_expr->span : node->span, "out of memory validating enum dot constructor");
            return result;
        }
    }
    ...
    for (arg_index = 0; arg_index < positional_count; arg_index++) {
        ...
        if (arg_index >= field_count) {
            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, call_expr->span, "too many positional arguments for enum variant '%s.%s'", enum_name, variant_name);
            break;
        }
        ...
        arg_info = zt_checker_check_expression(checker, call_expr->as.call_expr.positional_args.items[arg_index], scope, fn_ctx, field_type);
        if (!zt_checker_same_or_contextually_assignable(checker, scope, field_type, &arg_info, call_expr->as.call_expr.positional_args.items[arg_index]->span)) {
            zt_checker_diag_type(checker, ZT_DIAG_TYPE_MISMATCH, call_expr->as.call_expr.positional_args.items[arg_index]->span, "enum payload type mismatch", field_type, arg_info.type);
        }
    ...
    for (arg_index = 0; arg_index < named_count; arg_index++) {
        const zt_ast_named_arg *arg = &call_expr->as.call_expr.named_args.items[arg_index];
        size_t f;
        int found = 0;
    ...
    for (arg_index = 0; arg_index < field_count; arg_index++) {
        if (used_fields != NULL && !used_fields[arg_index]) {
            ...
            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, call_expr != NULL ? call_expr->span : node->span, "missing payload field '%s' in enum variant '%s.%s'", field_name != NULL ? field_name : "<field>", enum_name, variant_name);
        }
    }
...
static zt_expr_info zt_checker_check_call_expr(zt_checker *checker, const zt_ast_node *node, zt_binding_scope *scope, zt_function_context *fn_ctx, const zt_type *expected_type) {
    ...
    if (callee->kind == ZT_AST_ENUM_DOT_EXPR) {
        zt_expr_info_dispose(&result);
        return zt_checker_check_enum_dot_expr(checker, callee, scope, fn_ctx, expected_type, node);
    }
    ...
    if (callee->kind == ZT_AST_IDENT_EXPR) {
        ...
        /* R3.M5 Phase 2: indirect call through a local callable binding. */
        {
            zt_binding *value_binding = zt_binding_scope_lookup(scope, name, ZT_BINDING_VALUE);
            if (value_binding != NULL && value_binding->type != NULL &&
                value_binding->type->kind == ZT_TYPE_CALLABLE) {
                const zt_type *callable_type = value_binding->type;
                size_t expected_params = zt_type_callable_param_count(callable_type);
                size_t positional_count = node->as.call_expr.positional_args.count;
                size_t named_count = node->as.call_expr.named_args.count;
                size_t ai;
                int signature_mismatch = 0;
                zt_type_list actual_params = zt_type_list_make();
                zt_type *actual_signature = NULL;

                zt_type_dispose(result.type);
                result.type = zt_type_clone(zt_type_callable_return(callable_type));

                if (named_count > 0) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, node->span,
                        "named arguments are not supported when calling through a callable in v1");
                    return result;
                }
                for (ai = 0; ai < positional_count; ai++) {
                    const zt_type *pt = ai < expected_params ? zt_type_callable_param(callable_type, ai) : NULL;
                    zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[ai], scope, fn_ctx, pt);
                    zt_type_list_push(&actual_params, arg_info.type != NULL ? zt_type_clone(arg_info.type) : zt_type_make(ZT_TYPE_UNKNOWN));
                    if (ai < expected_params &&
                        !zt_checker_same_or_contextually_assignable(checker, scope, pt, &arg_info, node->as.call_expr.positional_args.items[ai]->span)) {
                        signature_mismatch = 1;
                    }
                    zt_expr_info_dispose(&arg_info);
                }
                actual_signature = zt_type_make_callable(zt_type_clone(zt_type_callable_return(callable_type)), actual_params);
                if (positional_count != expected_params) {
                    zt_checker_diag_callable_signature(checker, node->span, name, callable_type, actual_signature);
                    zt_type_dispose(actual_signature);
                    return result;
                }
        ...
        if (name != NULL && strcmp(name, "to_text") == 0) {
            zt_expr_info arg_info;
            zt_type_dispose(result.type);
            result.type = zt_type_make(ZT_TYPE_TEXT);
            if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "to_text(...) expects exactly one positional argument");
                return result;
            }

            arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
            if (arg_info.type == NULL ||
                !zt_checker_type_implements_trait(checker, scope, arg_info.type, "TextRepresentable")) {
                char type_buf[256];
                zt_type_describe(arg_info.type, type_buf, sizeof(type_buf));
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "to_text(...) requires TextRepresentable<T>; got '%s'", type_buf);
            }
            zt_expr_info_dispose(&arg_info);
            return result;
        }
        ...
        if (name != NULL && strcmp(name, "check") == 0) {
            zt_type *expected_bool = zt_type_make(ZT_TYPE_BOOL);
            zt_type *expected_text = NULL;
            zt_expr_info arg_info;
            zt_type_dispose(result.type);
            result.type = zt_type_make(ZT_TYPE_VOID);
            if (node->as.call_expr.named_args.count != 0 ||
                    node->as.call_expr.positional_args.count < 1 ||
                    node->as.call_expr.positional_args.count > 2) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "check(...) expects condition and optional text message");
                zt_type_dispose(expected_bool);
                return result;
            }
            arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, expected_bool);
            if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_BOOL) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "check(...) requires a bool argument");
            }
            zt_expr_info_dispose(&arg_info);
            if (node->as.call_expr.positional_args.count == 2) {
                expected_text = zt_type_make(ZT_TYPE_TEXT);
                arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[1], scope, fn_ctx, expected_text);
                if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_TEXT) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "check(...) optional message must be text");
                }
                zt_expr_info_dispose(&arg_info);
                zt_type_dispose(expected_text);
            }
            zt_type_dispose(expected_bool);
            return result;
        }
        ...
        if (name != NULL &&
            ...
            zt_type *expected_text = zt_type_make(ZT_TYPE_TEXT);
            zt_expr_info arg_info;
            zt_type_dispose(result.type);
            result.type = zt_type_make(ZT_TYPE_VOID);
            if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "%s(...) expects exactly one text message", name);
                zt_type_dispose(expected_text);
                return result;
            }
            arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, expected_text);
            if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_TEXT) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "%s(...) requires a text message", name);
            }
            zt_expr_info_dispose(&arg_info);
            zt_type_dispose(expected_text);
            return result;
        ...
        if (name != NULL && strcmp(name, "print") == 0) {
            zt_type_dispose(result.type);
            result.type = zt_type_make(ZT_TYPE_VOID);
            if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "print(...) expects exactly one positional argument");
                return result;
            }
            {
                ...
                if (arg_info.type == NULL ||
                    ...
                    char type_buf[256];
                    zt_type_describe(arg_info.type, type_buf, sizeof(type_buf));
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "print(...) requires TextRepresentable<T>; got '%s'", type_buf);
                }
                zt_expr_info_dispose(&arg_info);
            }
            return result;
        ...
        if (name != NULL && strcmp(name, "read") == 0) {
            ...
            if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 0) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "read() expects no arguments");
            }
            return result;
        }

        if (name != NULL && strcmp(name, "debug") == 0) {
            zt_type_dispose(result.type);
            result.type = zt_type_make(ZT_TYPE_VOID);
            if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "debug(...) expects exactly one positional argument");
                return result;
            }
            {
                zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
        ...
        if (name != NULL && strcmp(name, "type_name") == 0) {
            ...
            result.type = zt_type_make(ZT_TYPE_TEXT);
            if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "type_name(...) expects exactly one positional argument");
                return result;
            }
            {
                zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
        ...
        if (name != NULL && strcmp(name, "range") == 0) {
            zt_type *expected_int = zt_type_make(ZT_TYPE_INT);
            zt_type_list range_args = zt_type_list_make();
            zt_type_list_push(&range_args, zt_type_make(ZT_TYPE_INT));
            zt_type_dispose(result.type);
            result.type = zt_type_make_with_args(ZT_TYPE_LIST, NULL, range_args);
            if (node->as.call_expr.named_args.count != 0 ||
                    node->as.call_expr.positional_args.count < 2 ||
                    node->as.call_expr.positional_args.count > 3) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "range(...) expects 2 or 3 positional int arguments");
                zt_type_dispose(expected_int);
                return result;
            }
            for (i = 0; i < node->as.call_expr.positional_args.count; i++) {
                zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[i], scope, fn_ctx, expected_int);
                if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_INT) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->as.call_expr.positional_args.items[i]->span, "range(...) requires int arguments");
                }
                zt_expr_info_dispose(&arg_info);
            }
            zt_type_dispose(expected_int);
        ...
        if (name != NULL && strcmp(name, "size_of") == 0) {
            zt_type_dispose(result.type);
            result.type = zt_type_make(ZT_TYPE_INT);
            if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "size_of(...) expects exactly one positional argument");
                return result;
            }
            {
                zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
    ...
    if (callee->kind == ZT_AST_FIELD_EXPR &&
        ...
        strcmp(callee->as.field_expr.object->as.ident_expr.name, "core") == 0 &&
        strcmp(callee->as.field_expr.field_name, "Error") == 0) {
        zt_type *expected_text = zt_type_make(ZT_TYPE_TEXT);
        zt_expr_info arg_info;
        zt_type_dispose(result.type);
        result.type = zt_type_make(ZT_TYPE_CORE_ERROR);

        if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 2) {
            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "core.Error(...) expects exactly two positional arguments: code, message");
            zt_type_dispose(expected_text);
            return result;
        }
        ...
        arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, expected_text);
        if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_TEXT) {
            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->as.call_expr.positional_args.items[0]->span, "core.Error code must be text");
        }
        zt_expr_info_dispose(&arg_info);

        arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[1], scope, fn_ctx, expected_text);
        if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_TEXT) {
            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->as.call_expr.positional_args.items[1]->span, "core.Error message must be text");
        }
        zt_expr_info_dispose(&arg_info);
    ...
    if (callee->kind == ZT_AST_FIELD_EXPR) {
        ...
        if (callee->as.field_expr.object->kind == ZT_AST_IDENT_EXPR &&
            ...
            if (member_decl != NULL &&
                !zt_checker_decl_is_public(member_decl) &&
                (import_path != NULL && strncmp(import_path, "std.", 4) != 0) &&
                !zt_checker_is_self_prefix(checker, alias)) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "member '%s' is not public", combined_name);
                zt_expr_info_dispose(&object_info);
            ...
            if (import_path != NULL && strcmp(import_path, "std.list") == 0 && member != NULL) {
                if (strcmp(member, "is_empty") == 0) {
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.list.is_empty(...) expects exactly one positional list argument");
                    } else {
                        zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_LIST) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.list.is_empty(...) requires a list<T> argument");
                        }
                        zt_expr_info_dispose(&arg_info);
                    }
                    zt_type_dispose(result.type);
                ...
                if (strcmp(member, "first") == 0 || strcmp(member, "last") == 0) {
                    zt_type *return_type = NULL;
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.list.%s(...) expects exactly one positional list argument", member);
                    } else {
                        zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_LIST || arg_info.type->args.count != 1) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.list.%s(...) requires a list<T> argument", member);
                        } else {
                            zt_type_list optional_args = zt_type_list_make();
                            zt_type_list_push(&optional_args, zt_type_clone(arg_info.type->args.items[0]));
                            return_type = zt_type_make_with_args(ZT_TYPE_OPTIONAL, "optional", optional_args);
                        }
                        zt_expr_info_dispose(&arg_info);
                    }
                    zt_type_dispose(result.type);
                    result.type = return_type != NULL ? return_type : zt_type_make(ZT_TYPE_UNKNOWN);
                    zt_expr_info_dispose(&object_info);
                    return result;
                }

                if (strcmp(member, "rest") == 0) {
                    zt_type *return_type = NULL;
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.list.rest(...) expects exactly one positional list argument");
                    } else {
                        zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_LIST || arg_info.type->args.count != 1) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.list.rest(...) requires a list<T> argument");
                        } else {
                            return_type = zt_type_clone(arg_info.type);
                        }
                        zt_expr_info_dispose(&arg_info);
                    }
                    zt_type_dispose(result.type);
                ...
                if (strcmp(member, "skip") == 0) {
                    zt_type *return_type = NULL;
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 2) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.list.skip(...) expects exactly one list and one count argument");
                    } else {
                        zt_type *int_type = zt_type_make(ZT_TYPE_INT);
                        zt_expr_info list_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        zt_expr_info count_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[1], scope, fn_ctx, int_type);
                        if (list_info.type == NULL || list_info.type->kind != ZT_TYPE_LIST || list_info.type->args.count != 1) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.list.skip(...) requires a list<T> first argument");
                        } else {
                            return_type = zt_type_clone(list_info.type);
                        }
                        if (count_info.type == NULL || !zt_type_is_integral(count_info.type)) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->as.call_expr.positional_args.items[1]->span, "std.list.skip count must be integral");
                        }
                        zt_expr_info_dispose(&list_info);
                        zt_expr_info_dispose(&count_info);
            ...
            if (import_path != NULL && strcmp(import_path, "std.map") == 0 && member != NULL) {
                if (strcmp(member, "is_empty") == 0) {
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.map.is_empty(...) expects exactly one positional map argument");
                    } else {
                        zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_MAP) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.map.is_empty(...) requires a map<K,V> argument");
                        }
                        zt_expr_info_dispose(&arg_info);
                    }
                    zt_type_dispose(result.type);
                ...
                if (strcmp(member, "has_key") == 0) {
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 2) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.map.has_key(...) expects exactly one map and one key argument");
                    } else {
                        zt_expr_info map_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (map_info.type == NULL || map_info.type->kind != ZT_TYPE_MAP || map_info.type->args.count != 2) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.map.has_key(...) requires a map<K,V> first argument");
                        } else {
                            zt_expr_info key_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[1], scope, fn_ctx, map_info.type->args.items[0]);
                            if (!zt_checker_same_or_contextually_assignable(checker, scope, map_info.type->args.items[0], &key_info, node->as.call_expr.positional_args.items[1]->span)) {
                                zt_checker_diag_type(
                                    checker,
                                    ZT_DIAG_TYPE_MISMATCH,
                                    node->as.call_expr.positional_args.items[1]->span,
                                    "std.map.has_key key type mismatch",
                                    map_info.type->args.items[0],
                                    key_info.type);
                            }
                            zt_expr_info_dispose(&key_info);
                        }
                        zt_expr_info_dispose(&map_info);
                    }
                    zt_type_dispose(result.type);
                    result.type = zt_type_make(ZT_TYPE_BOOL);
                    zt_expr_info_dispose(&object_info);
            ...
            if (import_path != NULL && strcmp(import_path, "std.set") == 0 && member != NULL) {
                if (strcmp(member, "empty") == 0) {
                    zt_type *return_type = NULL;
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 0) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.set.empty() expects no arguments");
                    }
                    if (expected_type != NULL && expected_type->kind == ZT_TYPE_SET && expected_type->args.count == 1) {
                        return_type = zt_type_clone(expected_type);
                    } else {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "std.set.empty() requires an expected set<T> type");
                    }
                    zt_type_dispose(result.type);
                    result.type = return_type != NULL ? return_type : zt_type_make(ZT_TYPE_UNKNOWN);
                    zt_expr_info_dispose(&object_info);
                ...
                if (strcmp(member, "of") == 0) {
                    zt_type_list args = zt_type_list_make();
                    zt_type *element_type = NULL;
                    zt_type *return_type;
                    size_t arg_index;
                    if (node->as.call_expr.named_args.count != 0) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.set.of(...) uses positional values only");
                    }
                    if (expected_type != NULL &&
                            expected_type->kind == ZT_TYPE_SET &&
                            expected_type->args.count == 1 &&
                            expected_type->args.items[0] != NULL) {
                        element_type = zt_type_clone(expected_type->args.items[0]);
                    }
                    for (arg_index = 0; arg_index < node->as.call_expr.positional_args.count; arg_index += 1) {
                        zt_expr_info value_info = zt_checker_check_expression(
                            checker,
                            node->as.call_expr.positional_args.items[arg_index],
                            scope,
                            fn_ctx,
                            element_type);
                        if (element_type == NULL) {
                            element_type = zt_type_clone(value_info.type);
                        } else if (!zt_checker_same_or_contextually_assignable(
                                checker,
                                scope,
                                element_type,
                                &value_info,
                                node->as.call_expr.positional_args.items[arg_index]->span)) {
                            zt_checker_diag_type(
                                checker,
                                ZT_DIAG_TYPE_MISMATCH,
                                node->as.call_expr.positional_args.items[arg_index]->span,
                                "std.set.of element type mismatch",
                                element_type,
                                value_info.type);
                        }
                    ...
                    if (element_type == NULL) {
                        if (expected_type != NULL && expected_type->kind == ZT_TYPE_SET && expected_type->args.count == 1) {
                            ...
                            element_type = zt_type_make(ZT_TYPE_UNKNOWN);
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "std.set.of() with no values requires an expected set<T> type");
                        }
                    }
                    zt_type_list_push(&args, element_type);
                    return_type = zt_type_make_with_args(ZT_TYPE_SET, NULL, args);
                    if (return_type != NULL &&
                            return_type->args.count == 1 &&
                            return_type->args.items[0] != NULL &&
                            return_type->args.items[0]->kind != ZT_TYPE_UNKNOWN &&
                            (!zt_checker_type_implements_trait(checker, scope, return_type->args.items[0], "Hashable") ||
                             !zt_checker_type_implements_trait(checker, scope, return_type->args.items[0], "Equatable"))) {
                        char key_buf[256];
                        zt_type_describe(return_type->args.items[0], key_buf, sizeof(key_buf));
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_MAP_KEY_TYPE, node->span, "set element type '%s' must implement Hashable and Equatable", key_buf);
                    }
                    zt_type_dispose(result.type);
                    result.type = return_type != NULL ? return_type : zt_type_make(ZT_TYPE_UNKNOWN);
                ...
                if (strcmp(member, "is_empty") == 0) {
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.set.is_empty(...) expects exactly one positional set argument");
                    } else {
                        zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_SET) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.set.is_empty(...) requires a set<T> argument");
                        }
                        zt_expr_info_dispose(&arg_info);
                    }
                    zt_type_dispose(result.type);
                ...
                if (strcmp(member, "len") == 0) {
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.set.len(...) expects exactly one positional set argument");
                    } else {
                        zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (arg_info.type == NULL || arg_info.type->kind != ZT_TYPE_SET) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.set.len(...) requires a set<T> argument");
                        }
                        zt_expr_info_dispose(&arg_info);
                    }
                    zt_type_dispose(result.type);
                ...
                if (strcmp(member, "add") == 0) {
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 2) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.set.add(...) expects exactly one set and one value argument");
                    } else {
                        zt_expr_info set_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (set_info.type == NULL || set_info.type->kind != ZT_TYPE_SET || set_info.type->args.count != 1) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.set.add(...) requires a set<T> first argument");
                        } else {
                            zt_expr_info val_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[1], scope, fn_ctx, set_info.type->args.items[0]);
                            if (!zt_checker_expression_is_mutable_target(node->as.call_expr.positional_args.items[0], scope, fn_ctx)) {
                                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->as.call_expr.positional_args.items[0]->span, "std.set.add(...) requires a mutable set variable");
                            }
                            if (!zt_checker_same_or_contextually_assignable(checker, scope, set_info.type->args.items[0], &val_info, node->as.call_expr.positional_args.items[1]->span)) {
                                zt_checker_diag_type(
                                    checker,
                                    ZT_DIAG_TYPE_MISMATCH,
                                    node->as.call_expr.positional_args.items[1]->span,
                                    "std.set.add value type mismatch",
                                    set_info.type->args.items[0],
                                    val_info.type);
                            }
                            zt_expr_info_dispose(&val_info);
                        }
                        zt_expr_info_dispose(&set_info);
                    }
                    zt_type_dispose(result.type);
                    result.type = zt_type_make(ZT_TYPE_VOID);
                    zt_expr_info_dispose(&object_info);
                ...
                if (strcmp(member, "remove") == 0) {
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 2) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.set.remove(...) expects exactly one set and one value argument");
                    } else {
                        zt_expr_info set_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (set_info.type == NULL || set_info.type->kind != ZT_TYPE_SET || set_info.type->args.count != 1) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.set.remove(...) requires a set<T> first argument");
                        } else {
                            zt_expr_info val_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[1], scope, fn_ctx, set_info.type->args.items[0]);
                            if (!zt_checker_expression_is_mutable_target(node->as.call_expr.positional_args.items[0], scope, fn_ctx)) {
                                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->as.call_expr.positional_args.items[0]->span, "std.set.remove(...) requires a mutable set variable");
                            }
                            if (!zt_checker_same_or_contextually_assignable(checker, scope, set_info.type->args.items[0], &val_info, node->as.call_expr.positional_args.items[1]->span)) {
                                zt_checker_diag_type(
                                    checker,
                                    ZT_DIAG_TYPE_MISMATCH,
                                    node->as.call_expr.positional_args.items[1]->span,
                                    "std.set.remove value type mismatch",
                                    set_info.type->args.items[0],
                                    val_info.type);
                            }
                            zt_expr_info_dispose(&val_info);
                ...
                if (strcmp(member, "union") == 0 ||
                        strcmp(member, "intersect") == 0 ||
                        strcmp(member, "difference") == 0) {
                    zt_type *return_type = NULL;
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 2) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.set.%s(...) expects exactly two set arguments", member);
                    } else {
                        zt_expr_info left_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        zt_expr_info right_info = zt_checker_check_expression(
                            checker,
                            node->as.call_expr.positional_args.items[1],
                            scope,
                            fn_ctx,
                            left_info.type != NULL && left_info.type->kind == ZT_TYPE_SET ? left_info.type : NULL);

                        if (left_info.type == NULL ||
                                left_info.type->kind != ZT_TYPE_SET ||
                                left_info.type->args.count != 1 ||
                                right_info.type == NULL ||
                                right_info.type->kind != ZT_TYPE_SET ||
                                right_info.type->args.count != 1) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.set.%s(...) requires set<T> arguments", member);
                        } else if (!zt_checker_same_or_contextually_assignable(
                                checker,
                                scope,
                                left_info.type,
                            ...
                            zt_checker_diag_type(
                                ...
                                ZT_DIAG_TYPE_MISMATCH,
                                node->as.call_expr.positional_args.items[1]->span,
                                "std.set operation requires both sets to use the same element type",
                                left_info.type,
                                right_info.type);
                        } else {
                            return_type = zt_type_clone(left_info.type);
                        }
                ...
                if (strcmp(member, "has") == 0) {
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 2) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.set.has(...) expects exactly one set and one value argument");
                    } else {
                        zt_expr_info set_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, NULL);
                        if (set_info.type == NULL || set_info.type->kind != ZT_TYPE_SET || set_info.type->args.count != 1) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_TYPE_MISMATCH, node->span, "std.set.has(...) requires a set<T> first argument");
                        } else {
                            zt_expr_info val_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[1], scope, fn_ctx, set_info.type->args.items[0]);
                            if (!zt_checker_same_or_contextually_assignable(checker, scope, set_info.type->args.items[0], &val_info, node->as.call_expr.positional_args.items[1]->span)) {
                                zt_checker_diag_type(
                                    ...
                                    ZT_DIAG_TYPE_MISMATCH,
                                    node->as.call_expr.positional_args.items[1]->span,
                                    "std.set.has value type mismatch",
                                    set_info.type->args.items[0],
                                    val_info.type);
                            }
                            zt_expr_info_dispose(&val_info);
            ...
            if (import_path != NULL && strcmp(import_path, "std.concurrent") == 0 && member != NULL) {
                zt_type *copy_type = zt_checker_make_std_concurrent_copy_type(member);
                if (copy_type != NULL) {
                    int valid_copy_call = 1;
                    if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->span, "std.concurrent.%s(...) expects exactly one positional argument", member);
                        valid_copy_call = 0;
                    } else {
                        zt_expr_info arg_info = zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, copy_type);
                        if (!zt_checker_same_or_contextually_assignable(checker, scope, copy_type, &arg_info, node->as.call_expr.positional_args.items[0]->span)) {
                            zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CALL, node->as.call_expr.positional_args.items[0]->span, "std.concurrent.%s(...) cannot copy a value of this type", member);
                            valid_copy_call = 0;
                        }
                        zt_expr_info_dispose(&arg_info);
                    }
        ...
        if (object_info.type != NULL &&
            object_info.type->kind == ZT_TYPE_OPTIONAL &&
            object_info.type->args.count == 1 &&
            object_info.type->args.items[0] != NULL &&
            callee->as.field_expr.field_name != NULL) {
            const char *method_name = callee->as.field_expr.field_name;
            const zt_type *inner_type = object_info.type->args.items[0];
            ...
            if (strcmp(method_name, "is_some") == 0 || strcmp(method_name, "is_none") == 0) {
                if (node->as.call_expr.positional_args.count != 0 || node->as.call_expr.named_args.count != 0) {
                    zt_diag_list_add(
                        ...
                        node->span,
                        "optional.%s() expects no arguments",
                        method_name);
                }
            ...
            if (strcmp(method_name, "or") == 0) {
                if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                    zt_diag_list_add(
                        ...
                        ZT_DIAG_INVALID_CALL,
                        node->span,
                        "optional.or(...) expects exactly one positional fallback argument");
                } else {
                    zt_expr_info fallback_info =
                        zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, inner_type);
                    if (!zt_checker_same_or_contextually_assignable(checker, scope, inner_type, &fallback_info, node->as.call_expr.positional_args.items[0]->span)) {
                        zt_checker_diag_type(
                            checker,
                            ZT_DIAG_TYPE_MISMATCH,
                            node->as.call_expr.positional_args.items[0]->span,
                            "optional.or fallback type mismatch",
                            inner_type,
                            fallback_info.type);
                    }
                    zt_expr_info_dispose(&fallback_info);
                }

                zt_type_dispose(result.type);
                result.type = zt_type_clone(inner_type);
                zt_expr_info_dispose(&object_info);
                return result;
            }
            ...
            if (strcmp(method_name, "or_return") == 0) {
                if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                    zt_diag_list_add(
                        ...
                        ZT_DIAG_INVALID_CALL,
                        node->span,
                        "optional.or_return(...) expects exactly one positional return value");
                } else if (fn_ctx == NULL || fn_ctx->return_type == NULL || fn_ctx->return_type->kind == ZT_TYPE_VOID) {
                    zt_diag_list_add(
                        &checker->result->diagnostics,
                        ZT_DIAG_INVALID_CALL,
                        node->span,
                        "optional.or_return(...) requires an enclosing function with a return value");
                } else {
                    zt_expr_info return_info =
                        zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, fn_ctx->return_type);
                    if (!zt_checker_same_or_contextually_assignable(checker, scope, fn_ctx->return_type, &return_info, node->as.call_expr.positional_args.items[0]->span)) {
                        zt_checker_diag_type(
                            checker,
                            ZT_DIAG_TYPE_MISMATCH,
                            node->as.call_expr.positional_args.items[0]->span,
                            "optional.or_return return value type mismatch",
                            fn_ctx->return_type,
                            return_info.type);
                    }
                    zt_expr_info_dispose(&return_info);
                }
        ...
        if (object_info.type != NULL &&
            object_info.type->kind == ZT_TYPE_RESULT &&
            ...
            if (strcmp(method_name, "or_wrap") == 0) {
                ...
                if (object_info.type->args.items[1] == NULL ||
                        object_info.type->args.items[1]->kind != ZT_TYPE_CORE_ERROR) {
                    zt_diag_list_add(
                        &checker->result->diagnostics,
                        ZT_DIAG_INVALID_CALL,
                        node->span,
                        "result.or_wrap(...) is supported only for result<T, core.Error>");
                }
                ...
                if (node->as.call_expr.named_args.count != 0 || node->as.call_expr.positional_args.count != 1) {
                    zt_diag_list_add(
                        ...
                        ZT_DIAG_INVALID_CALL,
                        node->span,
                        "result.or_wrap(...) expects exactly one positional text context");
                } else {
                    zt_expr_info context_info =
                        zt_checker_check_expression(checker, node->as.call_expr.positional_args.items[0], scope, fn_ctx, text_type);
                    if (context_info.type == NULL || context_info.type->kind != ZT_TYPE_TEXT) {
                        zt_checker_diag_type(
                            checker,
                            ZT_DIAG_TYPE_MISMATCH,
                            node->as.call_expr.positional_args.items[0]->span,
                            "result.or_wrap context type mismatch",
                            text_type,
                            context_info.type);
                    }
                    zt_expr_info_dispose(&context_info);
                }

                zt_type_dispose(text_type);
                zt_type_dispose(result.type);
                result.type = zt_type_clone(object_info.type);
                zt_expr_info_dispose(&object_info);
                return result;
            }
        }
        ...
        if (method_decl != NULL) {
            ...
            if (method_decl->as.func_decl.is_mutating &&
                !zt_checker_expression_is_mutable_target(callee->as.field_expr.object, scope, fn_ctx)) {
                zt_diag_list_add(
                    &checker->result->diagnostics,
                    ZT_DIAG_INVALID_MUTATION,
                    node->span,
                    "mutating method '%s' requires a mutable receiver",
...
static zt_expr_info zt_checker_check_expression(zt_checker *checker, const zt_ast_node *node, zt_binding_scope *scope, zt_function_context *fn_ctx, const zt_type *expected_type) {
    ...
    switch (node->kind) {
        ...
        case ZT_AST_FMT_EXPR: {
            ...
            for (i = 0; i < node->as.fmt_expr.parts.count; i++) {
                ...
                zt_expr_info part_info;
                char part_type_buf[256];
                if (part == NULL || part->kind == ZT_AST_STRING_EXPR) continue;
                part_info = zt_checker_check_expression(checker, part, scope, fn_ctx, NULL);
                if (part_info.type != NULL &&
                    part_info.type->kind != ZT_TYPE_UNKNOWN &&
                    !zt_checker_type_implements_trait(checker, scope, part_info.type, "TextRepresentable")) {
                    zt_type_describe(part_info.type, part_type_buf, sizeof(part_type_buf));
                    zt_diag_list_add(
                        &checker->result->diagnostics,
                        ZT_DIAG_INVALID_TYPE,
                        part->span,
                        "fmt interpolation requires TextRepresentable<T>; got '%s'",
                        part_type_buf);
                }
                zt_expr_info_dispose(&part_info);
            }
            return info;
        }
        ...
        case ZT_AST_IDENT_EXPR:
            ...
            /* R3.M5 Phase 2: bare top-level function used as a callable value.
             * Allowed only when the expected type is ZT_TYPE_CALLABLE. */
            if (expected_type != NULL && expected_type->kind == ZT_TYPE_CALLABLE) {
                const zt_ast_node *fn_decl = zt_catalog_find_decl(&checker->catalog, node->as.ident_expr.name);
                if (fn_decl != NULL && fn_decl->kind == ZT_AST_FUNC_DECL) {
                    if (fn_decl->as.func_decl.type_params.count > 0) {
                        zt_checker_diag_invalid_callable_ref_expected(
                            checker,
                            node->span,
                            "generic function",
                    ...
                    if (fn_decl->as.func_decl.is_mutating) {
                        zt_checker_diag_invalid_callable_ref_expected(
                            checker,
                            node->span,
                            "mutating function",
                    ...
                    {
                        zt_type_list callable_params = zt_type_list_make();
                        zt_type *callable_ret;
                        size_t pi;
                        for (pi = 0; pi < fn_decl->as.func_decl.params.count; pi++) {
                            const zt_ast_node *param = fn_decl->as.func_decl.params.items[pi];
                            zt_type_list_push(&callable_params, zt_checker_resolve_type(checker, param->as.param.type_node, scope));
                        }
                        callable_ret = zt_checker_resolve_type(checker, fn_decl->as.func_decl.return_type, scope);
                        zt_type_dispose(info.type);
                        info.type = zt_type_make_callable(callable_ret, callable_params);
                    }
                }
            }
            return info;
        case ZT_AST_ENUM_DOT_EXPR:
            ...
            return zt_checker_check_enum_dot_expr(checker, node, scope, fn_ctx, expected_type, NULL);
        ...
        case ZT_AST_IF_EXPR: {
            ...
            zt_expr_info then_info;
            zt_expr_info else_info;
            zt_type *branch_type = NULL;

            cond_info = zt_checker_check_expression(checker, node->as.if_expr.condition, scope, fn_ctx, zt_type_make(ZT_TYPE_BOOL));
            if (cond_info.type == NULL || cond_info.type->kind != ZT_TYPE_BOOL) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_CONDITION_TYPE, node->as.if_expr.condition->span, "if expression condition must be bool");
            }

            then_info = zt_checker_check_expression(checker, node->as.if_expr.then_expr, scope, fn_ctx, expected_type);
            branch_type = expected_type != NULL ? zt_type_clone(expected_type) : zt_type_clone(then_info.type);
            else_info = zt_checker_check_expression(checker, node->as.if_expr.else_expr, scope, fn_ctx, branch_type);

            if (branch_type != NULL && !zt_checker_same_or_contextually_assignable(checker, scope, branch_type, &then_info, node->as.if_expr.then_expr->span)) {
                zt_checker_diag_type(checker, ZT_DIAG_TYPE_MISMATCH, node->as.if_expr.then_expr->span, "if expression then branch type mismatch", branch_type, then_info.type);
            }
            if (branch_type != NULL && !zt_checker_same_or_contextually_assignable(checker, scope, branch_type, &else_info, node->as.if_expr.else_expr->span)) {
                zt_checker_diag_type(checker, ZT_DIAG_TYPE_MISMATCH, node->as.if_expr.else_expr->span, "if expression else branch type mismatch", branch_type, else_info.type);
            }

            zt_type_dispose(info.type);
            info.type = branch_type != NULL ? zt_type_clone(branch_type) : zt_type_make(ZT_TYPE_UNKNOWN);

            zt_type_dispose(branch_type);
            zt_expr_info_dispose(&cond_info);
            zt_expr_info_dispose(&then_info);
            zt_expr_info_dispose(&else_info);
            return info;
        }
        ...
        case ZT_AST_STRUCT_LITERAL_EXPR: {
            ...
            zt_type_dispose(info.type);
            info.type = expected_type != NULL ? zt_type_clone(expected_type) : zt_type_make(ZT_TYPE_UNKNOWN);

            if (expected_type != NULL && expected_type->kind == ZT_TYPE_MAP) {
                const zt_type *expected_key = expected_type->args.count >= 1 ? expected_type->args.items[0] : NULL;
                const zt_type *expected_value = expected_type->args.count >= 2 ? expected_type->args.items[1] : NULL;
                for (i = 0; i < node->as.struct_literal_expr.fields.count; i++) {
                    const zt_ast_named_arg *arg = &node->as.struct_literal_expr.fields.items[i];
                    zt_ast_node key_node;
                    zt_expr_info key_info;
                    zt_expr_info value_info;
                    memset(&key_node, 0, sizeof(key_node));
                    key_node.kind = ZT_AST_IDENT_EXPR;
                    key_node.span = arg->span;
                    key_node.as.ident_expr.name = arg->name;

                    key_info = zt_checker_check_expression(checker, &key_node, scope, fn_ctx, expected_key);
                    value_info = zt_checker_check_expression(checker, arg->value, scope, fn_ctx, expected_value);
                    if (expected_key != NULL &&
                            !zt_checker_same_or_contextually_assignable(checker, scope, expected_key, &key_info, arg->span)) {
                        zt_checker_diag_type(checker, ZT_DIAG_TYPE_MISMATCH, arg->span, "map key type mismatch", expected_key, key_info.type);
                    }
                    if (expected_value != NULL &&
                            !zt_checker_same_or_contextually_assignable(checker, scope, expected_value, &value_info, arg->span)) {
                        zt_checker_diag_type(checker, ZT_DIAG_TYPE_MISMATCH, arg->span, "map value type mismatch", expected_value, value_info.type);
                    }
                    zt_expr_info_dispose(&key_info);
                    zt_expr_info_dispose(&value_info);
                }
            ...
            if (expected_type == NULL || expected_type->kind != ZT_TYPE_USER) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "struct literal requires an expected struct type");
                for (i = 0; i < node->as.struct_literal_expr.fields.count; i++) {
                    zt_expr_info field_info = zt_checker_check_expression(checker, node->as.struct_literal_expr.fields.items[i].value, scope, fn_ctx, NULL);
                    zt_expr_info_dispose(&field_info);
                }
                return info;
            }

            struct_decl = zt_catalog_find_decl(&checker->catalog, zt_type_base_name(expected_type));
            if (struct_decl == NULL || struct_decl->kind != ZT_AST_STRUCT_DECL) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, node->span, "struct literal expected '%s', but no struct declaration was found", zt_type_base_name(expected_type));
                return info;
            }

            if (struct_decl->as.struct_decl.fields.count > sizeof(used_fields) / sizeof(used_fields[0])) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, node->span, "struct literal has too many fields to validate");
                return info;
            }

            memset(used_fields, 0, sizeof(used_fields));
            zt_binding_scope_init(&type_scope, scope);
            zt_checker_bind_struct_field_type_params(&type_scope, struct_decl, expected_type);
            ...
            for (i = 0; i < node->as.struct_literal_expr.fields.count; i++) {
                ...
                for (f = 0; f < struct_decl->as.struct_decl.fields.count; f++) {
                    ...
                    if (used_fields[f]) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, arg->span, "field '%s' passed more than once in struct literal", arg->name);
                    }
                    used_fields[f] = 1;
                    field_type = zt_checker_resolve_type(checker, field->as.struct_field.type_node, &type_scope);
                    value_info = zt_checker_check_expression(checker, arg->value, scope, fn_ctx, field_type);
                    if (!zt_checker_same_or_contextually_assignable(checker, scope, field_type, &value_info, arg->span)) {
                        zt_checker_diag_type(checker, ZT_DIAG_TYPE_MISMATCH, arg->span, "struct literal field type mismatch", field_type, value_info.type);
                    }
                    zt_expr_info_dispose(&value_info);
                    zt_type_dispose(field_type);
                    found = 1;
                ...
                if (!found) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, arg->span, "unknown field '%s' in struct literal", arg->name);
                }
            }

            for (i = 0; i < struct_decl->as.struct_decl.fields.count; i++) {
                const zt_ast_node *field = struct_decl->as.struct_decl.fields.items[i];
                if (field != NULL && !used_fields[i] && field->as.struct_field.default_value == NULL) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, node->span, "missing field '%s' in struct literal", field->as.struct_field.name);
                }
            }
        ...
        case ZT_AST_SET_EXPR: {
            ...
            for (i = 0; i < node->as.set_expr.elements.count; i++) {
                zt_expr_info element_info = zt_checker_check_expression(checker, node->as.set_expr.elements.items[i], scope, fn_ctx, element_type);
                if (element_type == NULL) {
                    element_type = zt_type_clone(element_info.type);
                } else if (!zt_checker_same_or_contextually_assignable(checker, scope, element_type, &element_info, node->as.set_expr.elements.items[i]->span)) {
                    zt_checker_diag_type(checker, ZT_DIAG_TYPE_MISMATCH, node->as.set_expr.elements.items[i]->span, "set element type mismatch", element_type, element_info.type);
                }
                zt_expr_info_dispose(&element_info);
            }
            if (element_type == NULL) element_type = zt_type_make(ZT_TYPE_UNKNOWN);
            zt_type_list_push(&args, element_type);
            zt_type_dispose(info.type);
            info.type = zt_type_make_with_args(ZT_TYPE_SET, NULL, args);
            if (!zt_checker_type_implements_trait(checker, scope, info.type->args.items[0], "Hashable") ||
                !zt_checker_type_implements_trait(checker, scope, info.type->args.items[0], "Equatable")) {
                char key_buf[256];
                zt_type_describe(info.type->args.items[0], key_buf, sizeof(key_buf));
        ...
        case ZT_AST_CLOSURE_EXPR: {
            ...
            if (node->as.closure_expr.return_type != NULL) {
                ...
                zt_checker_infer_closure_return_from_block(
                    checker,
                    node->as.closure_expr.body,
                    &closure_scope,
                    &closure_ctx,
                    &return_type_eval);
                if (return_type_eval == NULL) {
                    return_type_eval = zt_type_make(ZT_TYPE_VOID);
                }
            }
        ...
        switch (node->as.unary_expr.op) {
            ...
            case ZT_TOKEN_QUESTION:
                if (operand_info.type->kind == ZT_TYPE_OPTIONAL && operand_info.type->args.count == 1) {
                    if (fn_ctx == NULL ||
                            fn_ctx->return_type == NULL ||
                            fn_ctx->return_type->kind != ZT_TYPE_OPTIONAL ||
                            fn_ctx->return_type->args.count != 1) {
                        zt_diag_list_add(
                            &checker->result->diagnostics,
                            ZT_DIAG_INVALID_OPERATOR,
                            node->span,
                            "'?' on optional<T> requires an enclosing function that returns optional<U>");
                    }
                    zt_type_dispose(info.type);
    ...
    if (node->kind == ZT_AST_BINARY_EXPR) {
        ...
        switch (node->as.binary_expr.op) {
            ...
            case ZT_TOKEN_GT:
            case ZT_TOKEN_GTE:
                if (zt_checker_type_implements_trait(checker, scope, left_info.type, "Comparable") &&
                    (zt_checker_same_or_contextually_assignable(checker, scope, left_info.type, &right_info, node->as.binary_expr.right->span) ||
                     (zt_type_is_integral(right_info.type) && zt_expr_matches_integral_type(&left_info, right_info.type)) ||
                    ...
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_OPERATOR, node->span, "comparison requires compatible Comparable operands");
                }
                break;
            case ZT_TOKEN_AND:
            case ZT_TOKEN_OR:
...
static void zt_checker_check_statement(zt_checker *checker, const zt_ast_node *stmt, zt_binding_scope *scope, zt_function_context *fn_ctx) {
    ...
    switch (stmt->kind) {
        ...
        case ZT_AST_VAR_DECL:
            ...
            break;
        case ZT_AST_USING_STMT: {
            zt_binding_scope using_scope;
            expr_info = zt_checker_check_expression(checker, stmt->as.using_stmt.init_value, scope, fn_ctx, NULL);
            zt_binding_scope_init(&using_scope, scope);
            if (expr_info.type != NULL) {
                zt_binding_scope_declare(&using_scope, ZT_BINDING_VALUE, stmt->as.using_stmt.name, zt_type_clone(expr_info.type), 0);
            }
            if (stmt->as.using_stmt.cleanup_expr != NULL) {
                zt_checker_check_expression(checker, stmt->as.using_stmt.cleanup_expr, &using_scope, fn_ctx, NULL);
            }
            if (stmt->as.using_stmt.body != NULL) {
                zt_checker_check_block(checker, stmt->as.using_stmt.body, &using_scope, fn_ctx);
            } else if (expr_info.type != NULL) {
                zt_binding_scope_declare(scope, ZT_BINDING_VALUE, stmt->as.using_stmt.name, zt_type_clone(expr_info.type), 0);
            }
            zt_binding_scope_dispose(&using_scope);
            zt_expr_info_dispose(&expr_info);
            break;
        }
        ...
        case ZT_AST_FIELD_ASSIGN_STMT: {
            ...
            const zt_ast_node *import_member_decl = NULL;
            const char *import_alias = NULL;
            const char *import_path = NULL;
            char combined_name[512];
            int import_member_access_error = 0;
            int imported_module_var_mutation = 0;
            zt_binding_scope type_scope;

            import_member_decl = zt_checker_lookup_import_member(
                checker,
                stmt->as.field_assign_stmt.object,
            ...
            if (import_member_decl != NULL) {
                if (!zt_checker_decl_is_public(import_member_decl) &&
                    (import_path != NULL && strncmp(import_path, "std.", 4) != 0) &&
                    !zt_checker_is_self_prefix(checker, import_alias)) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_UNRESOLVED_NAME, stmt->span, "member '%s' is not public", combined_name);
                    import_member_access_error = 1;
                } else if (import_member_decl->kind == ZT_AST_VAR_DECL &&
                           !zt_checker_is_self_prefix(checker, import_alias)) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_MUTATION, stmt->span, "cannot mutate imported module variable '%s'", combined_name);
                    imported_module_var_mutation = 1;
                } else if (import_member_decl->kind == ZT_AST_VAR_DECL) {
                    field_type = zt_checker_resolve_type(checker, import_member_decl->as.var_decl.type_node, scope);
                } else if (import_member_decl->kind == ZT_AST_CONST_DECL) {
                    field_type = zt_checker_resolve_type(checker, import_member_decl->as.const_decl.type_node, scope);
                }
            }

            if (!import_member_access_error &&
                !imported_module_var_mutation &&
                !zt_checker_expression_is_mutable_target(stmt->as.field_assign_stmt.object, scope, fn_ctx)) {
            ...
            if (field_type == NULL && object_info.type != NULL && object_info.type->kind == ZT_TYPE_USER) {
        ...
        case ZT_AST_FOR_STMT: {
            ...
            if (iterable_info.type != NULL) {
                if (iterable_info.type->kind == ZT_TYPE_LIST && iterable_info.type->args.count == 1) {
                ...
                } else if (iterable_info.type->kind == ZT_TYPE_SET && iterable_info.type->args.count == 1) {
                    zt_binding_scope_declare(&loop_scope, ZT_BINDING_VALUE, stmt->as.for_stmt.item_name, zt_type_clone(iterable_info.type->args.items[0]), 1);
                    if (stmt->as.for_stmt.index_name != NULL) {
                        zt_binding_scope_declare(&loop_scope, ZT_BINDING_VALUE, stmt->as.for_stmt.index_name, zt_type_make(ZT_TYPE_INT), 1);
                    }
                    ...
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_TYPE, stmt->as.for_stmt.iterable->span, "for loop requires iterable of type list<T>, map<K, V>, set<T> or text");
                }
            }
        ...
        case ZT_AST_MATCH_STMT: {
            ...
            for (c = 0; c < stmt->as.match_stmt.cases.count; c++) {
                ...
                if (case_node->as.match_case.is_default) {
                    int duplicate_default = has_default;
                    if (has_default) {
                        zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, case_node->span, "duplicate case default in match");
                    }
                    has_default = 1;
                    if (subject_is_enum && subject_decl != NULL && !duplicate_default) {
                        zt_diag_list_add_severity(
                            &checker->result->diagnostics,
                            ZT_DIAG_ENUM_DEFAULT_CASE,
                            ZT_DIAG_SEVERITY_WARNING,
                            case_node->span,
                            "case default in match on enum '%s' hides explicit variant coverage",
                            subject_decl->as.enum_decl.name);
                    }
                ...
                if (subject_info.type != NULL && subject_info.type->kind == ZT_TYPE_OPTIONAL) {
                    for (p = 0; p < case_node->as.match_case.patterns.count; p++) {
                        const zt_ast_node *pattern = case_node->as.match_case.patterns.items[p];
                        if (pattern == NULL) continue;
                        ...
                        if (pattern->kind == ZT_AST_VALUE_BINDING) {
                            if (subject_info.type->args.count > 0 && subject_info.type->args.items[0] != NULL) {
                                ...
                                if (zt_binding_scope_lookup(&case_scope, pattern->as.value_binding.name, ZT_BINDING_VALUE) != NULL) {
                                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_INVALID_ARGUMENT, pattern->span, "duplicate or shadowed binding '%s' in match case", pattern->as.value_binding.name);
                                    zt_type_dispose(inner_type);
                                } else {
                                    zt_binding_scope_declare(&case_scope, ZT_BINDING_VALUE, pattern->as.value_binding.name, inner_type, 0);
                                }
                            } else {
                                zt_type *inner_type = zt_type_make(ZT_TYPE_UNKNOWN);
                                zt_binding_scope_declare(&case_scope, ZT_BINDING_VALUE, pattern->as.value_binding.name, inner_type, 0);
                            }
                            continue;
                        }

                        if (pattern->kind == ZT_AST_NONE_EXPR) {
                            continue;
                        }
            ...
            if (subject_is_enum && !has_default && subject_decl != NULL && subject_decl->kind == ZT_AST_ENUM_DECL) {
                ...
                if (!all_covered) {
                    char missing_cases[512];
                    zt_checker_format_missing_enum_cases(subject_decl, seen_variants, missing_cases, sizeof(missing_cases));
                    zt_diag_list_add(
                        &checker->result->diagnostics,
                        ZT_DIAG_NON_EXHAUSTIVE_MATCH,
                        stmt->span,
                        "match on enum '%s' is not exhaustive; missing case(s): %s",
                        subject_decl->as.enum_decl.name,
                        missing_cases[0] != '\0' ? missing_cases : "<unknown>");
                    zt_diag_list_add_severity(
                        &checker->result->diagnostics,
                        ZT_DIAG_NON_EXHAUSTIVE_MATCH,
                        ZT_DIAG_SEVERITY_NOTE,
                        stmt->span,
                        "add explicit cases for the missing variants or add 'case default' as the final case");
                }
            }

            if (!subject_is_enum && subject_info.type != NULL && subject_info.type->kind == ZT_TYPE_OPTIONAL && !has_default) {
                int has_value_case = 0;
                int has_none_case = 0;
                for (c = 0; c < stmt->as.match_stmt.cases.count; c++) {
                    const zt_ast_node *case_node = stmt->as.match_stmt.cases.items[c];
                    size_t p;
                    ...
                    for (p = 0; p < case_node->as.match_case.patterns.count; p++) {
                        const zt_ast_node *pattern = case_node->as.match_case.patterns.items[p];
                        if (pattern == NULL) continue;
                        if (pattern->kind == ZT_AST_VALUE_BINDING) has_value_case = 1;
                        if (pattern->kind == ZT_AST_NONE_EXPR) has_none_case = 1;
                    }
                }
                if (!has_value_case) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_NON_EXHAUSTIVE_MATCH, stmt->span, "match on optional is not exhaustive: missing 'case value' pattern");
                }
                if (!has_none_case) {
...
static void zt_checker_seed_module_value_bindings(zt_checker *checker, zt_binding_scope *scope) {
    size_t i;
    if (checker == NULL || scope == NULL) return;
    if (checker->catalog.value_decl_count == 0) return;
    for (i = 0; i < checker->catalog.decls.count; i++) {
        const zt_ast_node *decl = checker->catalog.decls.items[i].node;
        if (decl == NULL) continue;
        if (decl->kind == ZT_AST_CONST_DECL && decl->as.const_decl.name != NULL) {
            zt_binding_scope_declare(
                scope,
                ZT_BINDING_VALUE,
        ...
        if (decl->kind == ZT_AST_VAR_DECL && decl->as.var_decl.name != NULL) {
            zt_binding_scope_declare(
                scope,
                ZT_BINDING_VALUE,
                decl->as.var_decl.name,
                zt_checker_resolve_type(checker, decl->as.var_decl.type_node, scope),
                1);
            continue;
        }
    }
...
static void zt_checker_seed_module_value_aliases(zt_checker *checker, zt_binding_scope *scope, const char *decl_name) {
    char prefix[256];
    size_t prefix_len;
    size_t i;
    ...
    if (checker == NULL || scope == NULL) return;
    if (checker->catalog.value_decl_count == 0) return;
    if (!zt_checker_split_prefix(decl_name, prefix, sizeof(prefix))) return;
    ...
    for (i = 0; i < checker->catalog.decls.count; i++) {
        ...
        if (decl == NULL) continue;
        if (decl->kind == ZT_AST_CONST_DECL) {
            name = decl->as.const_decl.name;
            is_mutable = 0;
        ...
        if (strncmp(name, prefix, prefix_len) != 0 || name[prefix_len] != '.') continue;
        dot = strrchr(name, '.');
        if (dot == NULL || dot[1] == '\0') continue;
        short_name = dot + 1;

        if (zt_binding_scope_lookup(scope, short_name, ZT_BINDING_VALUE) != NULL) continue;

        if (decl->kind == ZT_AST_CONST_DECL) {
            decl_type = zt_checker_resolve_type(checker, decl->as.const_decl.type_node, scope);
        } else {
            decl_type = zt_checker_resolve_type(checker, decl->as.var_decl.type_node, scope);
        }
        zt_binding_scope_declare(scope, ZT_BINDING_VALUE, short_name, decl_type, is_mutable);
    }
}
...
static void zt_checker_check_decl(zt_checker *checker, const zt_ast_node *decl) {
    ...
    switch (decl->kind) {
        case ZT_AST_FUNC_DECL:
            zt_checker_seed_module_value_aliases(checker, &scope, decl->as.func_decl.name);
        ...
        case ZT_AST_STRUCT_DECL:
            ...
            for (i = 0; i < decl->as.struct_decl.fields.count; i++) {
                ...
                if (field_type != NULL && field_type->kind == ZT_TYPE_CALLABLE) {
                    zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_CALLABLE_ESCAPE_STRUCT_FIELD,
                        field->span, "field '%s' cannot hold a callable value in v1",
                        field->as.struct_field.name);
                }
        ...
        case ZT_AST_ENUM_DECL:
            ...
            break;
        case ZT_AST_TYPE_ALIAS_DECL:
            zt_type_dispose(zt_checker_resolve_type(checker, decl->as.type_alias_decl.target_type, &scope));
            break;
        case ZT_AST_APPLY_DECL: {
            ...
            if (decl->as.apply_decl.trait_name == NULL) {
                ...
                self_type = zt_checker_resolve_user_type(checker, decl->as.apply_decl.target_name, zt_type_list_make(), decl->span, &scope);
                ...
                self_type = zt_checker_resolve_user_type(checker, decl->as.apply_decl.target_name, target_args, decl->span, &scope);
            }
        ...
        case ZT_AST_EXTERN_DECL:
            ...
            break;
        case ZT_AST_CONST_DECL:
            zt_checker_seed_module_value_aliases(checker, &scope, decl->as.const_decl.name);
            decl_type = zt_checker_resolve_type(checker, decl->as.const_decl.type_node, &scope);
            ...
            break;
        case ZT_AST_VAR_DECL:
            zt_checker_seed_module_value_aliases(checker, &scope, decl->as.var_decl.name);
            decl_type = zt_checker_resolve_type(checker, decl->as.var_decl.type_node, &scope);
            if (decl_type != NULL && decl_type->kind == ZT_TYPE_CALLABLE && decl->as.var_decl.is_public) {
                zt_diag_list_add(&checker->result->diagnostics, ZT_DIAG_CALLABLE_ESCAPE_PUBLIC_VAR,
                    decl->span, "public var '%s' cannot hold a callable value in v1",
                    decl->as.var_decl.name != NULL ? decl->as.var_decl.name : "<anon>");
            }
            expr_info = zt_checker_check_expression(checker, decl->as.var_decl.init_value, &scope, NULL, decl_type);
            if (!zt_checker_same_or_contextually_assignable(checker, &scope, decl_type, &expr_info, decl->as.var_decl.init_value != NULL ? decl->as.var_decl.init_value->span : decl->span)) {
                zt_checker_diag_type(checker, ZT_DIAG_TYPE_MISMATCH, decl->span, "var initializer type mismatch", decl_type, expr_info.type);
            }
            zt_expr_info_dispose(&expr_info);
            zt_type_dispose(decl_type);
            break;
...
int zt_checker_type_is_transferable(const zt_ast_node *root, const zt_type *type) {
    ...
    zt_catalog_build(&checker.catalog, root);
    zt_binding_scope_init(&scope, NULL);
    zt_transfer_stack_init(&stack);

    is_transferable = zt_checker_type_is_transferable_inner(&checker, &scope, type, &stack);

    zt_transfer_stack_dispose(&stack);
    zt_binding_scope_dispose(&scope);
    zt_catalog_dispose(&checker.catalog);
