...
#define OUT_DIR ".ztc-tmp/tests/conformance-m14"
...
static void test_project_rejected(const char *project_dir, const char *name) {
    char build_cmd[1024];
    int code;

    snprintf(build_cmd, sizeof(build_cmd), DRIVER " build %s > " OUT_DIR "/%s.build.out 2>&1", project_dir, name);
    code = run_command(build_cmd);
    ASSERT_TRUE(code != 0, name);
}
...
int main(void) {
    ...
    test_behavior_project(
        ...
        "tests\\behavior\\multifile_import_alias\\build\\multifile-import-alias.exe",
        42,
