"""
R2.M2 - Parser fuzz driver.

Applies token/nesting-level mutators to corpus inputs to stress the parser.
Detects crashes/timeouts; persists offending seeds; optionally minimizes.
"""
...
from mutators import PARSER_MUTATORS, apply_random  # noqa: E402
...
def run_campaign(iters: int, seed: int, timeout_sec: float,
    ...
    for i in range(iters):
        ...
        k = rng.randint(1, 4)
        fuzzed = apply_random(rng, base, PARSER_MUTATORS, k=k)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Zenith parser fuzz harness")
