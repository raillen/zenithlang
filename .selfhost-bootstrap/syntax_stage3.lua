...
function Lexer:scan_identifier(start, line, col)
    while ((self:is_alpha(self:peek()) or self:is_digit(self:peek()))) do
        self:advance()
    end
...
function Parser:is_name_token(k)
    ...
    if ((((((k == TokenKind.KwFunc) or (k == TokenKind.KwVar)) or (k == TokenKind.KwIf)) or (k == TokenKind.KwElse)) or (k == TokenKind.KwElif))) then
        return true
    end
    if ((((((k == TokenKind.KwWhile) or (k == TokenKind.KwReturn)) or (k == TokenKind.KwPub)) or (k == TokenKind.KwStruct)) or (k == TokenKind.KwEnum))) then
        return true
    end
    if (((((k == TokenKind.KwSelf) or (k == TokenKind.KwAnd)) or (k == TokenKind.KwOr)) or (k == TokenKind.KwDo))) then
        return true
    end
    if ((((((k == TokenKind.KwEnd) or (k == TokenKind.KwFor)) or (k == TokenKind.KwIn)) or (k == TokenKind.KwRepeat)) or (k == TokenKind.KwMatch))) then
        return true
    end
    if ((((((k == TokenKind.KwCase) or (k == TokenKind.KwBreak)) or (k == TokenKind.KwContinue)) or (k == TokenKind.KwNamespace)) or (k == TokenKind.KwImport))) then
        return true
    end
    if (((((k == TokenKind.KwExport) or (k == TokenKind.KwAs)) or (k == TokenKind.KwNative)) or (k == TokenKind.KwExtern))) then
        return true
    end
    return false
end
...
function Parser:parse_declaration()
    ...
    local is_pub = false
    if (((self:peek().kind == TokenKind.KwPub) or (self:peek().kind == TokenKind.KwExport))) then
        self:advance()
        is_pub = true
    end
    local t = self:peek()
    ...
    if ((t.kind == TokenKind.KwFunc)) then
        ...
        if ((t.kind == TokenKind.KwVar)) then
            ...
            if ((t.kind == TokenKind.KwStruct)) then
                node = self:parse_struct_decl(is_pub)
            else
                if ((t.kind == TokenKind.KwEnum)) then
                    node = self:parse_enum_decl(is_pub)
                else
                    if ((t.kind == TokenKind.KwExtern)) then
...
function Parser:parse_assign_target()
    ...
    if ((start_t.kind == TokenKind.At)) then
        ...
        if ((start_t.kind == TokenKind.Identifier)) then
            ...
            if ((start_t.kind == TokenKind.KwSelf)) then
    ...
    while (true) do
        if ((self:peek().kind == TokenKind.Dot)) then
            ...
            if ((self:peek().kind == TokenKind.LBracket)) then
...
function Parser:parse_native_lua_stmt()
    ...
    while ((not self:is_at_end())) do
        ...
        local k = t.kind
        if (inline_body) then
            if (((((((k == TokenKind.KwIf) or (k == TokenKind.KwDo)) or (k == TokenKind.KwFunc)) or (k == TokenKind.KwWhile)) or (k == TokenKind.KwFor)) or (k == TokenKind.KwRepeat))) then
                depth = zt.add(depth, 1)
            else
                if ((k == TokenKind.KwEnd)) then
                    depth = (depth - 1)
                    if ((depth == 0)) then
                        break
                    end
            ...
            if (((k == TokenKind.KwEnd) and (t.span.column == start_t.span.column))) then
                break
            end
        end
        if ((code ~= "")) then
...
function Parser:parse_match_stmt()
    ...
    while (((not self:is_at_end()) and (self:peek().kind ~= TokenKind.KwEnd))) do
        if ((self:peek().kind == TokenKind.KwElse)) then
            ...
            if ((self:peek().kind == TokenKind.KwCase)) then
...
function Parser:parse_inline_statement_block(line_no)
    ...
    if ((((self:peek().kind == TokenKind.KwEnd) or (self:peek().kind == TokenKind.KwElse)) or (self:peek().kind == TokenKind.KwElif))) then
        return stmts
    end
    local s = self:parse_statement()
    if ((s ~= nil)) then
...
function Parser:parse_primary()
    local t = self:advance()
    local node = nil
    if (((t.kind == TokenKind.IntegerLiteral) or (t.kind == TokenKind.StringLiteral))) then
        ...
        if ((t.kind == TokenKind.Identifier)) then
            if ((t.text == "null")) then
                ...
                if (((t.text == "true") or (t.text == "false"))) then
                    node = ExprNode.new({["kind"] = NodeKind.LiteralExpr, ["value"] = t.text, ["span"] = t.span, ["left"] = nil, ["right"] = nil, ["operator"] = nil, ["name"] = "", ["callee"] = nil, ["args"] = nil, ["object"] = nil, ["index"] = nil, ["symbol"] = nil})
                else
            ...
            if ((t.kind == TokenKind.KwSelf)) then
                ...
                if (self:is_name_token(t.kind)) then
                    ...
                    if ((t.kind == TokenKind.LParen)) then
                        ...
                        if ((t.kind == TokenKind.At)) then
                            ...
                            if ((t.kind == TokenKind.LBracket)) then
                                node = self:parse_list_literal(t)
                            else
                                if ((t.kind == TokenKind.LBrace)) then
                                    node = self:parse_map_literal(t)
                                else
                                    if (((t.kind == TokenKind.KwNot) or (t.kind == TokenKind.Minus))) then
                                        ...
                                        if ((t.kind == TokenKind.KwNative)) then
                                            ...
                                            while ((not self:is_at_end())) do
                                                ...
                                                local nk = nt.kind
                                                if (((((((nk == TokenKind.KwIf) or (nk == TokenKind.KwDo)) or (nk == TokenKind.KwFunc)) or (nk == TokenKind.KwWhile)) or (nk == TokenKind.KwFor)) or (nk == TokenKind.KwRepeat))) then
                                                    depth = zt.add(depth, 1)
                                                else
                                                    if ((nk == TokenKind.KwEnd)) then
                                                        depth = (depth - 1)
                                                        if ((depth == 0)) then
                                                            break
                                                        end
    ...
    while (true) do
        ...
        if ((next.kind == TokenKind.LParen)) then
            ...
            if ((next.kind == TokenKind.Dot)) then
                self:advance()
                ...
                if ((next.kind == TokenKind.LBracket)) then
                    self:advance()
                    local idx = self:parse_expression(0)
                    self:expect(TokenKind.RBracket, "esperado ']'")
                    ...
                    if ((next.kind == TokenKind.KwAs)) then
                        self:advance()
                        self:parse_type()
                    else
                        break
...
function Parser:get_precedence(k)
    if (((k == TokenKind.Plus) or (k == TokenKind.Minus))) then
        return 7
    end
    if (((k == TokenKind.Star) or (k == TokenKind.Slash))) then
        return 8
    end
    if (((k == TokenKind.EqualEqual) or (k == TokenKind.BangEqual))) then
        return 4
    end
    if (((((k == TokenKind.Less) or (k == TokenKind.LessEqual)) or (k == TokenKind.Greater)) or (k == TokenKind.GreaterEqual))) then
        return 5
    end
...
function scope_has_local_symbol(sc, name)
    return symbol_is_present(sc.symbols[name])
end
local Scope = {}
Scope.__index = Scope
...
function ModuleManager:resolve_path(module_name)
    local path = ""
    -- native lua
    if module_name : sub ( 1 , 4 ) == "std." then path = "src/stdlib/" .. module_name : sub ( 5 ) : gsub ( "%." , "/" ) .. ".zt" else path = self . project_root .. "/" .. module_name : gsub ( "%." , "/" ) .. ".zt" end
    return path
end
...
function Binder:_is_platform_allowed(n)
    if (((n == nil) or (n.attributes == nil))) then
        return true
    end
    ...
    while (has_slot(n.attributes, i)) do
        ...
        if ((((a == "windows") or (a == "linux")) or (a == "macos"))) then
...
function Binder:_declare(n)
    if (((((n.kind == NodeKind.FuncDecl) or (n.kind == NodeKind.VarDecl)) or (n.kind == NodeKind.StructDecl)) or (n.kind == NodeKind.EnumDecl))) then
        local k = 0
        ...
        if ((n.kind == NodeKind.VarDecl)) then
            ...
            if ((n.kind == NodeKind.IdentifierExpr)) then
                ...
                if ((n.kind == NodeKind.BinaryExpr)) then
                    ...
                    if ((n.kind == NodeKind.ReturnStmt)) then
                        ...
                        if ((n.kind == NodeKind.ExternDecl)) then
                            ...
                            if ((n.kind == NodeKind.ExprStmt)) then
                                self:_bind_node(n.value)
                            else
                                if ((n.kind == NodeKind.AssignStmt)) then
                                    self:_bind_node(n.target)
                                    self:_bind_node(n.value)
                                else
                                    if ((n.kind == NodeKind.ForInStmt)) then
                                        ...
                                        if ((n.kind == NodeKind.RepeatStmt)) then
                                            ...
                                            if ((n.kind == NodeKind.MatchStmt)) then
                                                ...
                                                if ((n.kind == NodeKind.MemberExpr)) then
...
function Emitter:generate(u)
    ...
    if (has_slot(imps, 0)) then
        ...
        while (has_slot(imps, i)) do
            ...
            -- native lua
            if lua_path : sub ( 1 , 4 ) == "std." then lua_path = "src.stdlib." .. lua_path : sub ( 5 ) end
        ...
        if ((n.kind == NodeKind.VarDecl)) then
            ...
            if ((n.kind == NodeKind.ReturnStmt)) then
                ...
                if ((n.kind == NodeKind.AssignStmt)) then
                    ...
                    if ((n.kind == NodeKind.IfStmt)) then
                        ...
                        if ((n.kind == NodeKind.WhileStmt)) then
                                                ...
                                                if ((n.kind == NodeKind.EnumDecl)) then
                                                    ...
                                                    if (has_slot(mems, 0)) then
                                                        ...
                                                        while (has_slot(mems, i)) do
                                                            self:emit(zt.add(zt.add(zt.add(mems[i], " = \""), mems[i]), "\","))
                                                            i = zt.add(i, 1)
                                                        end
                                                    end
                                                    self:dedent()
...
function Emitter:expr_to_lua(n)
    ...
    if ((n.kind == NodeKind.BinaryExpr)) then
        ...
        if ((n.kind == NodeKind.CallExpr)) then
            ...
            if ((n.kind == NodeKind.MemberExpr)) then
                ...
                if ((n.kind == NodeKind.IndexExpr)) then
                    ...
                    if ((n.kind == NodeKind.UnaryExpr)) then
                        ...
                        if ((n.kind == NodeKind.ListLiteral)) then
                            ...
                            if ((n.kind == NodeKind.MapLiteral)) then
                                local res = lbrace_text()
                                local keys = n.keys
                                local vals = n.values
                                ...
                                if ((n.kind == NodeKind.StructInitExpr)) then
                                    ...
                                    if (has_slot(keys, 0)) then
                                        local i = 0
                                        while (has_slot(keys, i)) do
                                            res = zt.add(zt.add(zt.add(zt.add(res, "[\""), keys[i]), "\"] = "), self:expr_to_lua(vals[i]))
                                            if (has_slot(keys, zt.add(i, 1))) then
                                                res = zt.add(res, ", ")
                                            end
                                            i = zt.add(i, 1)
                                    ...
                                    if ((n.kind == NodeKind.SelfExpr)) then
                                        return "self"
                                    else
                                        if ((n.kind == NodeKind.NativeLuaExpr)) then
