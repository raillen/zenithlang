...
-- Suporte a Sleep Nativo para o Scheduler
local native_sleep = function() end
local has_ffi, ffi = pcall(require, "ffi")
if has_ffi then
    if is_windows then
        ffi.cdef[[ void Sleep(uint32_t dwMilliseconds); ]]
        local kernel32 = ffi.load("kernel32")
        native_sleep = function(ms) kernel32.Sleep(ms) end
    end
end
...
local string_methods = string_mt.__index

string_mt.__index = function(self, k)
    if type(k) == "number" then
        -- Zenith v0.2 usa indexação 0-based para strings e listas.
        local c = self:sub(k + 1, k + 1)
...
string_methods.starts_with = function(self, prefix)
    return self:sub(1, #prefix) == prefix
end

string_methods.ends_with = function(self, suffix)
    return suffix == "" or self:sub(-#suffix) == suffix
end

string_methods.contains = function(self, search)
...
function zt.await(task)
    ...
    while not task.is_done() do
        ...
        coroutine.yield()
        -- Note: o scheduler (zt.drive) chamará o step da tarefa que estamos esperando
    end
    
    return task.result
end

--- Driver principal para rodar o scheduler assíncrono.
function zt.drive(main_task)
    while true do
        local active = 0
        for i = #tasks, 1, -1 do
            local t = tasks[i]
        ...
        -- Se não houver mais tarefas ativas, o sistema pousa.
        if active == 0 then break end
        
        -- Pequena folga para a CPU para evitar 100% de uso
        native_sleep(1)
    end
    return main_task and main_task.result
end
