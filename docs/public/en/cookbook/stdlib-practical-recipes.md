...
## Recipe: Return Errors From a CLI Helper

Use when a command can fail.

Recommended example:

```text
examples/cli-calculator/
```

Core shape:

```zt
func calculate(left: int, right: int) -> result<int, core.Error>
    if right == 0
...
Do not use `panic` for normal user input errors.

Return `result` so the caller can decide what to do.
