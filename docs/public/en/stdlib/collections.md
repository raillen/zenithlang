...
Removes a key. Returns the updated map (COW).
...
Creates an empty ordered set. Does not allow duplicates.
...
Inserts an element. If it already exists, the set does not change.
...
Removes an element. Returns the updated set.
