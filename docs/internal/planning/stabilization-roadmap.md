...
| Bootstrap self-hosted | verificador historico de bootstrap | OK; stage1/stage2/stage3 carregam e stage2/stage3 sao identicos |
...
| SH-5 | Definir caminho verificavel de bootstrap confiavel | Concluida | O verificador historico de bootstrap checava `syntax.zt`, gerava `stage1`, `stage2` e `stage3`, validava load e confirmava determinismo no fluxo de `.selfhost-artifacts/bootstrap/summary.txt` |
...
- Plano operacional: consolidado nesta secao.
...
- O verificador historico de bootstrap era nao-destrutivo e orientado a `src/compiler/syntax.zt`.
- O fluxo gerava `syntax_stage1.lua`, `syntax_stage2.lua` e `syntax_stage3.lua`, validava `loadfile` em cada etapa e gravava resumo em `.selfhost-artifacts/bootstrap/summary.txt`.
...
O ciclo posterior ao fechamento da Trilha B foi consolidado neste roadmap.
