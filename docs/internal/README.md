# Internal Docs

> Documentacao de evolucao, manutencao e implementacao do repositorio.
> Audience: contributor, maintainer
> Surface: internal

## Objetivo

Manter planejamento, evidencia e arquitetura interna fora das docs publicas.

Use esta pasta para:

- roadmaps;
- checklists;
- relatorios;
- auditorias;
- governance;
- standards;
- arquitetura interna;
- arquivo historico.

## Secoes

- `planning/`: roadmaps, checklists e planos por ciclo.
- `reports/`: evidencia, auditorias, release reports e triage curada.
- `governance/`: baselines, politicas de aceite e qualidade.
- `standards/`: padroes editoriais e regras de documentacao.
- `architecture/`: visao tecnica interna.
- `decisions/`: decisoes internas que nao pertencem a `language/decisions/`.
- `archive/`: historico e material bruto.
