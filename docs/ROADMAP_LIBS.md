...
## 🌀 Fase 2: Utilidades e Tempo (v0.2.6)
...
## 🛠️ Fase 3: Tooling e Ecossistema (v0.2.7) - *Concluído*
*Foco: Gestão de pacotes e experiência do desenvolvedor.*

### [x] ZPM (Zenith Package Manager)
- [x] Interface CLI com estilo "Zenith Aura".
- [x] Comandos base (Estrutura): `init`, `add`, `install`, `remove`, `update`, `run`, `list`, `clean`, `doctor`, `info`, `publish`.
- [x] Lógica de `zpm install`: Download em massa de dependências.
- [x] Lógica de `zpm info`: Star Profile de pacotes.
- [x] Lógica de `zpm update`: Sincronia de versões via Git.
- [x] Lógica de `zpm clean`: Faxina de órbita.
- [x] Suporte a dependências via Git/HTTP.
- [x] Geração de `zenith.lock` via `std.json`.
- [x] Gestão de pacotes globais e locais.

---

## 🚀 Fase 4: Avançado e Conectividade (v0.2.8)
*Foco: Poder computacional, segurança e comunicação.*

### [x] `std.math`
- **Descrição**: Cálculos avançados e constantes matemáticas fundamentais.
- **Core**: [x] `sin`, [x] `cos`, [x] `tan`, [x] `asin`, [x] `acos`, [x] `atan2`, [x] `sqrt`, [x] `abs`, [x] `round`, [x] `floor`, [x] `ceil`, [x] `pow`.
- **Constantes**: [x] `PI`, [x] `E`, [x] `TAU`.
- **Utilidades**: [x] `clamp(val, min, max)`, [x] `lerp(a, b, t)`.
- **Submódulo `std.math.random`**:
    - [x] `random.seed(val)`
    - [x] `random.int(min, max)`
    - [x] `random.float(min, max)`
    - [x] `random.bool()`
- **Submódulo `std.math.vec`**:
    - [x] `struct Vec2`: `x`, [x] `y`, [x] `length()`, [x] `normalize()`, [x] `dot()`.
    - [x] Construtor `vec2(x, y)`.

### [x] `std.collections`
- **Descrição**: Estruturas de dados além das listas básicas com segurança de iteração.
- **Queue (FIFO)**: [x] `enqueue_item`, [x] `dequeue_item`, [x] `peek_front`.
- **Stack (LIFO)**: [x] `push_item`, [x] `pop_item`, [x] `peek_top`.
- **Set (Únicos)**: [x] `add_item`, [x] `remove_item`, [x] `contains_item`.
- **MultiMap**: [ ] `add_pair(key, value)`, `get_values(key)`.
- **Utilidades de Lista**:
    - [x] `filter_items(list, predicate)`
    - [x] `map_items(list, transform)`
    - [x] `chunk_list(list, size)`
    - [x] `distinct_items(list)`
    - [x] `find_first(list, predicate)`

### [/] `std.crypto`
- **Descrição**: Segurança e codificação de dados.
- **Funcionalidades**: 
    - [x] Base64 (Encode/Decode).
    - [x] Hashes (MD5, SHA256).
    - [x] HMAC-SHA256.
    - [x] UUID v4.
    - [x] Helper `hash_password`.

### [ ] `std.log`
- **Descrição**: Sistema de registro visual para depuração.
- **Funcionalidades**: 
    - [x] Níveis (`trace`, `debug`, `info`, `warn`, `error`, `panic`).
    - [x] Cores Aura (ANSI) e metadados de arquivo/linha.
    - [x] Formato estruturado (JSON).
    - [ ] TODO: Desenvolver sistema de Sinks (UI, Banco de Dados, Coletores externos).

### [ ] `std.net` (Baixo Nível)
- **Descrição**: Comunicação bruta via rede (TCP/UDP).
- **Funcionalidades**: 
    - [ ] `socket.tcp_client()` (Async connect/send/receive).
    - [ ] `socket.tcp_server()` (Async accept).
    - [ ] `struct Buffer`: Manipulação eficiente de bytes.
    - [ ] Resolução automática de DNS.
    - [ ] TODO: Suporte a UDP (`udp_client`, `udp_server`).

### [ ] `std.http` (Alto Nível)
- **Descrição**: Cliente e Servidor Web modernos e assíncronos.
- **HTTP Client**:
    - [x] `get`, `post`, `put`, `delete`, `patch` (Async).
    - [x] `http.download_file` com progresso.
- **HTTP Server**:
    - [ ] Sistema de Rotas (`on_get`, `on_post`).
    - [ ] Middlewares (Filtros de requisição).
    - [ ] `serve_folder` (Arquivos estáticos).
    - [ ] Suporte a WebSockets.
    - [ ] Auto-JSON (Serialização automática de structs).

### [x] `std.reflect`
- **Descrição**: Introspecção e manipulação dinâmica de tipos.
- **Funcionalidades**: 
    - [x] `type_of(obj)`
    - [x] `inspect(obj)` (Campos e Métodos)
    - [x] `get_value / set_value` dinâmico.

### [x] `std.events`
- **Descrição**: Comunicação desacoplada entre módulos.
- **Funcionalidades**: `EventEmitter` (on, emit, once, wait_for).
- **Status**: v0.2.5 (Concluído)

### [x] `std.os.process` (Submódulo)
- **Descrição**: Gestão avançada de processos do sistema.
- **Funcionalidades**: 
    - [x] Spawn em background.
    - [x] Captura de `stdout/stderr`.
    - [x] Controle via `await p.wait()`.
