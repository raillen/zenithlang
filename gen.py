...
code = """#include "compiler/tooling/formatter.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
...
for kind in ast_kinds:
    code += f"        case {kind}:\n"
    if kind == "ZT_AST_FILE":
        code += """            if (node->as.file.module_name) {
...
"""
    elif kind == "ZT_AST_NAMESPACE_DECL":
        code += """            sb_append(sb, "namespace ");
            sb_append(sb, node->as.namespace_decl.name);
"""
    elif kind == "ZT_AST_IMPORT_DECL":
        code += """            sb_append(sb, "import ");
...
"""
    elif kind == "ZT_AST_FUNC_DECL":
        code += """            if (node->as.func_decl.is_public) sb_append(sb, "public ");
            sb_append(sb, "func ");
...
"""
    elif kind == "ZT_AST_STRUCT_DECL":
        code += """            if (node->as.struct_decl.is_public) sb_append(sb, "public ");
            sb_append(sb, "struct ");
            ...
            sb_append(sb, "}");
"""
    elif kind == "ZT_AST_TRAIT_DECL":
        code += """            if (node->as.trait_decl.is_public) sb_append(sb, "public ");
            sb_append(sb, "trait ");
            ...
            sb_append(sb, "}");
"""
    elif kind == "ZT_AST_APPLY_DECL":
        code += """            sb_append(sb, "apply ");
            ...
            sb_append(sb, "}");
"""
    elif kind == "ZT_AST_ENUM_DECL":
        code += """            if (node->as.enum_decl.is_public) sb_append(sb, "public ");
            sb_append(sb, "enum ");
            ...
            sb_append(sb, "}");
"""
    elif kind == "ZT_AST_EXTERN_DECL":
        code += """            if (node->as.extern_decl.is_public) sb_append(sb, "public ");
            sb_append(sb, "extern ");
            ...
            sb_append(sb, "}");
"""
    elif kind == "ZT_AST_STRUCT_FIELD":
        code += """            sb_append(sb, node->as.struct_field.name);
...
"""
    elif kind == "ZT_AST_TRAIT_METHOD":
        code += """            sb_append(sb, "func ");
...
"""
    elif kind == "ZT_AST_ENUM_VARIANT":
        code += """            sb_append(sb, node->as.enum_variant.name);
...
"""
    elif kind == "ZT_AST_GENERIC_CONSTRAINT":
        code += """            sb_append(sb, node->as.generic_constraint.type_param_name);
            sb_append(sb, " is ");
            format_node(sb, node->as.generic_constraint.trait_type);
"""
    elif kind == "ZT_AST_PARAM":
        code += """            sb_append(sb, node->as.param.name);
...
"""
    elif kind == "ZT_AST_TYPE_SIMPLE":
        code += """            sb_append(sb, node->as.type_simple.name);
"""
    elif kind == "ZT_AST_TYPE_GENERIC":
        code += """            sb_append(sb, node->as.type_generic.name);
...
"""
        ...
        code += """            sb_append(sb, "{\\n");
            ...
            sb_append(sb, "}");
"""
    elif kind == "ZT_AST_IF_STMT":
        code += """            sb_append(sb, "if ");
...
"""
    elif kind == "ZT_AST_WHILE_STMT":
        code += """            sb_append(sb, "while ");
            format_node(sb, node->as.while_stmt.condition);
            sb_append(sb, " ");
            format_node(sb, node->as.while_stmt.body);
"""
    elif kind == "ZT_AST_FOR_STMT":
        code += """            sb_append(sb, "for ");
...
"""
    elif kind == "ZT_AST_REPEAT_STMT":
        code += """            sb_append(sb, "repeat ");
            format_node(sb, node->as.repeat_stmt.count);
            sb_append(sb, " times ");
            format_node(sb, node->as.repeat_stmt.body);
"""
    elif kind == "ZT_AST_RETURN_STMT":
        code += """            sb_append(sb, "return");
            if (node->as.return_stmt.value) {
                sb_append(sb, " ");
                format_node(sb, node->as.return_stmt.value);
            }
"""
    elif kind == "ZT_AST_VAR_DECL":
        code += """            sb_append(sb, "var ");
...
"""
    elif kind == "ZT_AST_CONST_DECL":
        code += """            sb_append(sb, "const ");
...
"""
    elif kind == "ZT_AST_ASSIGN_STMT":
        code += """            sb_append(sb, node->as.assign_stmt.name);
            sb_append(sb, " = ");
            format_node(sb, node->as.assign_stmt.value);
"""
    elif kind == "ZT_AST_INDEX_ASSIGN_STMT":
        code += """            format_node(sb, node->as.index_assign_stmt.object);
            sb_append(sb, "[");
            format_node(sb, node->as.index_assign_stmt.index);
            sb_append(sb, "] = ");
            format_node(sb, node->as.index_assign_stmt.value);
"""
    elif kind == "ZT_AST_FIELD_ASSIGN_STMT":
        code += """            format_node(sb, node->as.field_assign_stmt.object);
            sb_append(sb, ".");
            sb_append(sb, node->as.field_assign_stmt.field_name);
            sb_append(sb, " = ");
            format_node(sb, node->as.field_assign_stmt.value);
"""
    elif kind == "ZT_AST_MATCH_STMT":
        code += """            sb_append(sb, "match ");
            ...
            sb_append(sb, "}");
"""
    elif kind == "ZT_AST_MATCH_CASE":
        code += """            if (node->as.match_case.is_default) {
...
"""
    elif kind == "ZT_AST_BREAK_STMT":
        code += """            sb_append(sb, "break");
"""
    elif kind == "ZT_AST_CONTINUE_STMT":
        code += """            sb_append(sb, "continue");
"""
    elif kind == "ZT_AST_EXPR_STMT":
        code += """            format_node(sb, node->as.expr_stmt.expr);
"""
    elif kind == "ZT_AST_BINARY_EXPR":
        code += """            format_node(sb, node->as.binary_expr.left);
            sb_append(sb, " ");
            sb_append(sb, op_to_str(node->as.binary_expr.op));
            sb_append(sb, " ");
            format_node(sb, node->as.binary_expr.right);
"""
    elif kind == "ZT_AST_UNARY_EXPR":
        code += """            sb_append(sb, op_to_str(node->as.unary_expr.op));
            if (isalpha(op_to_str(node->as.unary_expr.op)[0])) sb_append(sb, " ");
            format_node(sb, node->as.unary_expr.operand);
"""
    elif kind == "ZT_AST_CALL_EXPR":
        code += """            format_node(sb, node->as.call_expr.callee);
            sb_append(sb, "(");
            ...
            sb_append(sb, ")");
"""
    elif kind == "ZT_AST_FIELD_EXPR":
        code += """            format_node(sb, node->as.field_expr.object);
            sb_append(sb, ".");
            sb_append(sb, node->as.field_expr.field_name);
"""
    elif kind == "ZT_AST_INDEX_EXPR":
        code += """            format_node(sb, node->as.index_expr.object);
            sb_append(sb, "[");
            format_node(sb, node->as.index_expr.index);
            sb_append(sb, "]");
"""
    elif kind == "ZT_AST_SLICE_EXPR":
        code += """            format_node(sb, node->as.slice_expr.object);
            sb_append(sb, "[");
            ...
            sb_append(sb, "]");
"""
    elif kind == "ZT_AST_INT_EXPR":
        code += """            sb_append(sb, node->as.int_expr.value);
"""
    elif kind == "ZT_AST_FLOAT_EXPR":
        code += """            sb_append(sb, node->as.float_expr.value);
"""
    elif kind == "ZT_AST_STRING_EXPR":
        code += """            sb_append(sb, node->as.string_expr.value);
"""
    elif kind == "ZT_AST_BYTES_EXPR":
        code += """            sb_append(sb, node->as.bytes_expr.value);
"""
    elif kind == "ZT_AST_BOOL_EXPR":
        code += """            if (node->as.bool_expr.value) sb_append(sb, "true");
            else sb_append(sb, "false");
"""
    elif kind == "ZT_AST_NONE_EXPR":
        code += """            sb_append(sb, "none");
"""
    elif kind == "ZT_AST_SUCCESS_EXPR":
        code += """            sb_append(sb, "success(");
            if (node->as.success_expr.value) format_node(sb, node->as.success_expr.value);
            sb_append(sb, ")");
"""
    elif kind == "ZT_AST_ERROR_EXPR":
        code += """            sb_append(sb, "error(");
            if (node->as.error_expr.value) format_node(sb, node->as.error_expr.value);
            sb_append(sb, ")");
"""
    elif kind == "ZT_AST_LIST_EXPR":
        code += """            sb_append(sb, "[");
            format_node_list_comma(sb, node->as.list_expr.elements);
            sb_append(sb, "]");
"""
    elif kind == "ZT_AST_MAP_EXPR":
        code += """            sb_append(sb, "{");
            ...
            sb_append(sb, "}");
"""
    elif kind == "ZT_AST_IDENT_EXPR":
        code += """            sb_append(sb, node->as.ident_expr.name);
"""
    elif kind == "ZT_AST_GROUPED_EXPR":
        code += """            sb_append(sb, "(");
            format_node(sb, node->as.grouped_expr.inner);
            sb_append(sb, ")");
"""
    elif kind == "ZT_AST_WHERE_CLAUSE":
        code += """            sb_append(sb, "where ");
            sb_append(sb, node->as.where_clause.param_name);
            sb_append(sb, " -> ");
            format_node(sb, node->as.where_clause.condition);
"""
    elif kind == "ZT_AST_FORMAT_STRING_EXPR":
        code += """            sb_append(sb, "f\\\"format_string\\\""); // Simplified
"""
    elif kind == "ZT_AST_MATCH_BINDING":
        code += """            sb_append(sb, "todo_match_binding");
"""
...
with open("gen_formatter.py", "w") as f:
    pass

with open("formatter.c", "w", encoding="utf-8") as f:
    f.write(code)
